...
class _CEventsSale
{
	 ...
	 * Параметры <br>
	 * ID    Идентификатор заказа <br>
	 * Success    Индикатор успешности. Выводит true, если удаление произошло успешно. И false, если удаление заказа не произошло (например, не удалось удалить
	 * корзину заказа, или подписку по заказу).    Delete    4.0.6 <br>
	 */
	 ...
	 * bPay    Если параметр bWithdraw установлен в true, то установка параметра bPay в true приведет к тому, что необходимая сумма денег будет внесена на счет
	 * покупателя перед оплатой, а установка в false приведет к тому, что оплата будет происходить целиком с внутреннего счета; если параметр bWithdraw
	 * установлен в false, то операции со счетом не производятся и значение параметра bPay не играет роли. <br> recurringID    Должен быть равен 0 <br>
	 ...
	 * arModules    Массив идентификаторов модулей, которые должны принять список купонов. Если массив пустой, то обработчик события должен обработать все
	 * модули, которые имеют обработчик этого события.    CSaleBasket::DoSaveOrderBasket    11.5.0 <br>
	 */
	 ...
	 * intUserID    Идентификатор пользователя, для которого передаются купоны. <br>
	 * arCoupons    Один купон или массив передаваемых купонов. <br>
	 * arModules    Массив идентификаторов модулей, которые должны удалить из списка переданные купоны. Если массив пустой, то обработчик события должен
	 * обработать все модули, которые имеют обработчик этого события.    CSaleBasket::DoSaveOrderBasket    11.5.0 <br>
	 */
	 ...
	 * arModules    Массив идентификаторов модулей, из которых удаляется список купонов. Если массив пустой, то обработчик события должен обработать все
	 * модули, которые имеют обработчик этого события.    CSaleBasket::DoSaveOrderBasket    11.5.0 <br>
	 */
	...
	/**
	 * Вызывается перед отправкой письма о новом заказе, может быть использовано для модификации данных, изменения идентификатора типа почтового события, по
	 * которому будет осуществлена отправка, и отмены отправки письма. <br> Параметры <br> ID    Идентификатор заказа <br>
	...
	/**
	 * Вызывается перед отправкой письма о разрешении доставки заказа, может быть использовано для модификации данных, изменения идентификатора типа почтового
	 * события, по которому будет осуществлена отправка, и отмены отправки письма. <br> Параметры <br> ID    Идентификатор заказа <br>
	 * &eventName    Тип почтового события по которому будет осуществлена отправка <br>
	...
	/**
	 * Вызывается перед отправкой письма об оплате заказа, может быть использовано для модификации данных, изменения идентификатора типа почтового события, по
	 * которому будет осуществлена отправка, и отмены отправки письма. <br> Параметры <br> ID    Идентификатор заказа <br>
	 * &eventName    Тип почтового события по которому будет осуществлена отправка <br>
	...
	/**
	 * Вызывается перед отправкой письма об отмене заказа, может быть использовано для модификации данных, изменения идентификатора типа почтового события, по
	 * которому будет осуществлена отправка, и отмены отправки письма. <br> Параметры <br> ID    Идентификатор заказа <br>
	 * &eventName    Тип почтового события по которому будет осуществлена отправка <br>
	...
	/**
	 * Вызывается в момент формирования письма клиенту о смене статуса заказа. Может быть использовано для переопределения текста письма или его дополнения
	 * (для этого обработчик события должен возвращать необходимый текст). <br> Параметры <br> ID    Идентификатор заказа <br> val    Идентификатор статуса
	 * заказа    StatusOrder    4.0.6 <br>
	 */
	...
	/**
	 * Вызывается перед отправкой письма о cмене статуса заказа, может быть использовано для модификации данных, изменения идентификатора типа почтового
	 * события, по которому будет осуществлена отправка, и отмены отправки письма. <br> Параметры <br> ID    Идентификатор заказа <br>
	 * &eventName    Тип почтового события по которому будет осуществлена отправка <br>
	 * &arFields    Массив данных о заказе, которые будут подставлены в почтовый шаблон <br>
	...
	/**
	 * Вызывается перед отправкой письма о напоминании оплаты заказа, может быть использовано для модификации данных, изменения идентификатора типа почтового
	 * события, по которому будет осуществлена отправка, и отмены отправки письма. <br> Параметры <br> ID    Идентификатор заказа <br>
	 * &eventName    Тип почтового события по которому будет осуществлена отправка <br>
	 * &arFields    Массив данных о заказе, которые будут подставлены в почтовый шаблон <br>
	...
	/**
	 * Вызывается перед отправкой письма о добавлении заказа на продление подписки, может быть использовано для модификации данных, изменения идентификатора
	 * типа почтового события, по которому будет осуществлена отправка, и отмены отправки письма. <br> Параметры <br> ID    Идентификатор заказа <br>
	 * &eventName    Тип почтового события по которому будет осуществлена отправка <br>
	 * &arFields    Массив данных о заказе, которые будут подставлены в почтовый шаблон <br>
	 * CSaleOrder::RemindPayment    11.0.0 <br>
	 */
	...
	/**
	 * Вызывается перед отправкой письма об отмене заказа на продление подписки, может быть использовано для модификации данных, изменения идентификатора типа
	 * почтового события, по которому будет осуществлена отправка, и отмены отправки письма. <br> Параметры <br> ID    Идентификатор заказа <br>
	 * &eventName    Тип почтового события по которому будет осуществлена отправка <br>
	 * &arFields    Массив данных о заказе, которые будут подставлены в почтовый шаблон <br>
	...
	/**
	 * Вызывается в компоненте bitrix:sale.order.ajax после формирования списка доступных типов плательщика, может быть использовано для модификации данных.
	 * <br> Параметры <br>
	...
	/**
	 * Вызывается в компоненте bitrix:sale.order.ajax после формирования всех данных компонента на этапе заполнения формы заказа, может быть использовано для
	 * модификации данных. <br> Параметры <br>
	 * &arResult    Массив arResult компонента <br>
	 * &arUserResult    Массив arUserResult компонента, содержащий текущие выбранные пользовательские данные <br>
	 * arParams    Массив параметров компонента        11.0.0 <br>
	 */
	...
	/**
	 * Вызывается в компоненте bitrix:sale.order.ajax после создания заказа и всех его параметров, после отправки письма, но до редиректа на страницу с
	 * информацией о созданном заказе и оплате заказа. <br> Параметры <br> ID    Идентификатор заказа <br> arOrder    Массив полей заказа
	 * CSaleOrder::DoSaveOrder    11.0.0 <br>
	 */
	...
	/**
	 * Вызывается в компоненте bitrix:sale.order.ajax после создания заказа и всех его параметров, после отправки письма, перед выводом страницы об успешно
	 * созданном заказе и оплате заказа. <br> Параметры <br> ID    Идентификатор заказа <br> arOrder    Массив полей заказа <br> arParams    Массив параметров
	 * компонента        11.0.0 <br>
	 */
	...
	/**
	 * Вызывается в компоненте bitrix:sale.order.ajax перед подсчётом скидки при оформлении заказа. Можно использовать в том числе для присвоения/отъёма у
	 * пользователя купонов соответствующих скидок для расчёта индивидуальной скидки. <br> Параметры <br> arResult    Массив параметров заказа <br>
	 * arUserResult    Массив параметров пользователя <br> arParams    Массив параметров компонента        12.0.0 <br>
	 */
	...
	/**
	 * Вызывается при формировании фильтра для списка заказов в административной части. Позволяет модифицировать значения фильтра. Для этого обработчик события
	 * должен вернуть модифицированный массив фильтра. <br> Параметры <br> arFilter    Сформированный фильтр        5.1.0 <br>
	 */
	...
	/**
	 * Вызывается при формировании фильтра для выбора товаров для заказа. Позволяет модифицировать значения фильтра. Для этого обработчик события должен
	 * вернуть модифицированный массив фильтра. <br> Параметры <br> arFilter    Сформированный фильтр        5.1.0 <br>
	 */
	...
	/**
	 * Вызывается для каждого товара в форме поиска товара. Возвращаемое значение - массив (в формате JScript) новых параметров товара. Может использоваться,
	 * если товары в заказе должны иметь не те параметры, которые они имеют в каталоге. <br> Параметры <br> ID    Код товара <br> arParams    Текущее значение,
	 * передающееся в качестве параметров товара (массив в формате JScript)        5.1.0 <br>
	 */
	...
	/**
	 * Вызывается в методе CSaleAffiliate::CalculateAffiliate, если модуль товара не catalog. Позволяет задать секции для товара, расположенного не в модуле
	 * каталог, используемые для планов аффилиатов. <br> Параметры <br> MODULE    Идентификатор модуля товара корзины <br> PRODUCT_ID    Идентификатор товара
	 *  CSaleAffiliate::CalculateAffiliate    5.1.0
	 */
