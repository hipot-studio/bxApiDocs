...
class ormannotatecommand extends Command implements AnnotationInterface
{
            ...
            ->setDefinition(
                new InputDefinition([
                    ...
                    new InputOption(
                        ...
                        'Modules to be scanned, separated by comma.',
                        'main'
                    ),
                    new InputOption(
                        'clean',
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!empty($inputModulesRaw) && 'all' !== $inputModulesRaw) {
        ...
        // write to file
        $fileContent = '<?php'.PHP_EOL.PHP_EOL.implode(PHP_EOL, $annotations);
        ...
        $output->writeln('Scanned modules: '.implode(', ', $this->modulesScanned));
        ...
        $output->writeln('Memory usage: '.round($memoryAfter / 1_024 / 1_024, 1).'M (+'.round($memoryDiff / 1_024 / 1_024, 1).'M)');
        $output->writeln('Memory peak usage: '.round(memory_get_peak_usage() / 1_024 / 1_024, 1).'M');
    ...
    protected function handleVirtualClasses($inputModules, InputInterface $input, OutputInterface $output)
    {
        ...
        $event = new Event('main', 'onVirtualClassBuildList', [], $inputModules);
        $event->send();
