...
{
	/**
	 * Событие "OnPageStart" вызывается в начале выполняемой части пролога сайта, после подключения всех библиотек и отработки <a
	 * href="http://dev.1c-bitrix.ru/learning/course/index.php?COURSE_ID=43&amp;LESSON_ID=3436" >Агентов</a>.
	 *
	 *
	 * @return mixed
	 *
	...
	/**
	 * Событие "OnBeforeProlog" вызывается в выполняемой части пролога сайта (после события <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/events/onpagestart.php">OnPageStart</a>).
	 *
	 *
	 * @return mixed
	 *
	 ...
	 * }
	 * ?&gt;Смотрите также<li><a href="http://dev.1c-bitrix.ru/community/forums/messages/forum6/topic17229/message95191/#message95191">Как посчитать количество
	 *     пользователей онлайн</a></li>
	 * </pre>
	 *
	 *
	 * <h4>See Also</h4>
	...
	/**
	 * <p>Событие "OnAfterEpilog" возникает в конце выполняемой части эпилога сайта (после события <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/events/onepilog.php">OnEpilog</a>).  </p>
	 *
	 *
	 * @return mixed
	 *
	...
	/**
	 * <p>Событие OnAdminContextMenuShow вызывается в функции <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/general/admin.section/classes/cadmincontextmenu/show.php">CAdminContextMenu::Show()</a> при выводе в
	 * административном разделе панели кнопок. Событие позволяет модифицировать или добавить собственные кнопки на панель.</p>
	 *
	 *
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?<br>AddEventHandler("main", "OnAdminContextMenuShow", "MyOnAdminContextMenuShow");<br>function
	 *     MyOnAdminContextMenuShow(&amp;$items)<br>{<br>	//add custom button to the index page
	 *     toolbar<br>	if($GLOBALS["APPLICATION"]-&gt;GetCurPage(true) == "/bitrix/admin/index.php")<br>        $items[] = array("TEXT"=&gt;"Настройки
	 *     модулей", "ICON"=&gt;"", "TITLE"=&gt;"Страница настроек модулей", "LINK"=&gt;"settings.php?lang=".LANGUAGE_ID);<br>}<br>?&gt;
	 * </pre>
	 *
	 *
	 * <h4>See Also</h4>
	...
	/**
	 * <p>Событие OnAdminListDisplay вызывается в функции <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/general/admin.section/classes/cadminlist/display.php">CAdminList::Display()</a> при выводе в
	 * административном разделе списка элементов. Событие позволяет модифицировать объект списка, в частности, добавить произвольные групповые действия над
	 * элементами списка, добавить команды в меню действий элемента списка и т.п.</p>
	 *
	 *
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?<br>AddEventHandler("main", "OnAdminListDisplay", "MyOnAdminListDisplay");<br>function MyOnAdminListDisplay(&amp;$list)<br>{<br>	//add custom
	 *     group action<br>	if($list-&gt;table_id == "tbl_posting")<br>        $list-&gt;arActions["status_draft"] = "Статус: Черновик";<br>}<br>//process
	 *     custom action<br>AddEventHandler("main", "OnBeforeProlog", "MyOnBeforeProlog");<br>function
	 *     MyOnBeforeProlog()<br>{<br>	if($_SERVER["REQUEST_METHOD"] == "POST" &amp;&amp; $_POST["action"] == "status_draft" &amp;&amp;
	 *     is_array($_POST["ID"]) &amp;&amp; $GLOBALS["APPLICATION"]-&gt;GetCurPage() == "/bitrix/admin/posting_admin.php")<br>	{<br>
	 *     if($GLOBALS["APPLICATION"]-&gt;GetGroupRight("subscribe") == "W" &amp;&amp; check_bitrix_sessid())<br>        {<br>
	 *     if(CModule::IncludeModule("subscribe"))<br>            {<br>                $cPosting = new CPosting;<br>                foreach($_POST["ID"] as
	 *     $ID)<br>                    if(($ID = intval($ID)) &gt; 0)<br>                        $cPosting-&gt;ChangeStatus($ID, "D");<br>            }<br>
	 *        }<br>    }<br>}<br>?&gt;
	 * </pre>
	 *
	 *
	...
	/**
	 * <p>Событие OnAdminTabControlBegin вызывается в функции <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/general/admin.section/classes/cadmintabcontrol/begin.php">CAdminTabControl::Begin()</a> при выводе в
	 * административном интерфейсе формы редактирования. Событие позволяет изменить или добавить собственные вкладки формы редактирования.</p>
	 *
	 *
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?<br>AddEventHandler("main", "OnAdminTabControlBegin", "MyOnAdminTabControlBegin");<br>function
	 *     MyOnAdminTabControlBegin(&amp;$form)<br>{<br>	if($GLOBALS["APPLICATION"]-&gt;GetCurPage() == "/bitrix/admin/posting_edit.php")<br>	{<br>
	 *      $form-&gt;tabs[] = array("DIV" =&gt; "my_edit", "TAB" =&gt; "Дополнительно", "ICON"=&gt;"main_user_edit", "TITLE"=&gt;"Дополнительные параметры",
	 *     "CONTENT"=&gt;<br>            '&lt;tr valign="top"&gt;<br>                &lt;td&gt;Дополнительные заголовки письма:&lt;/td&gt;<br>
	 *     &lt;td&gt;<br>                    &lt;input type="text" name="MY_HEADERS[]" value="" size="30"&gt;&lt;br&gt;<br>                    &lt;input
	 *     type="text" name="MY_HEADERS[]" value="" size="30"&gt;&lt;br&gt;<br>                    &lt;input type="text" name="MY_HEADERS[]" value=""
	 *     size="30"&gt;&lt;br&gt;<br>                &lt;/td&gt;<br>            &lt;/tr&gt;'<br>        );<br>	}<br>}<br>?&gt;
	 * </pre>
	 *
	 *
	 * <h4>See Also</h4>
	...
	/**
	 * <p>Событие вызывается в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cgroup/add.php">CGroup::Add</a> после добавления группы,  и
	 * может быть использовано для действий, связанных с группой.</p>
	 *
	 *
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?<br>AddEventHandler("main", "OnAfterGroupAdd", "MyOnAfterGroupAdd");<br>function
	 *     MyOnAfterGroupAdd(&amp;$arFields)<br>{<br>	AddMessage2Log(print_r($arFields, true));<br>}?&gt;
	 * </pre>
	 *
	 *
	 * <h4>See Also</h4>
	...
	/**
	 * <p>Событие вызывается в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cgroup/update.php">CGroup::Update</a> после изменения полей
	 * группы,  и может быть использовано для дополнительных действий, связанных с группой.</p>
	 *
	 *
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?<br>AddEventHandler("main", "OnAfterGroupUpdate", "MyOnAfterGroupUpdate");<br>function MyOnAfterGroupUpdate($ID, &amp;$arFields)<br>{<br>	if($ID
	 *     == 1)<br>        AddMessage2Log(print_r($arFields, true));<br>}<br>?&gt;
	 * </pre>
	 *
	 *
	 * <h4>See Also</h4>
	...
	/**
	 * Событие "OnAfterUserAdd" вызывается после попытки добавления нового пользователя методом <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/add.php">CUser::Add</a>.
	 *
	 *
	...
	/**
	 * Обработчик события будет вызван из метода <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/authorize.php">CUser::Authorize</a> после
	 * авторизации пользователя, передавая в параметре <i>user_fields</i> массив всех полей авторизованного пользователя.
	 *
	 *
	...
	/**
	 * <p>Событие "OnAfterUserLogin" вызывается в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/login.php">CUser::Login</a> после
	 * попытки авторизовать пользователя, проверив имя входа <i><span class="syntax"><i>arParams</i></span>['LOGIN']</i> и пароль <i><span
	 * class="syntax"><i>arParams</i></span>['PASSWORD']</i>.</p>
	 *
	 *
	...
	/**
	 * Событие "OnAfterUserLoginByHash" вызывается в методе <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/loginbyhash.php">CUser::LoginByHash()</a> после проверки имени входа <i><span
	 * class="syntax"><i>arParams</i><b></b></span>['LOGIN']</i> и хеша от пароля <span class="syntax"><i>arParams</i></span><i>['HASH']</i> и попытки
	 * авторизовать пользователя. В параметре <span class="syntax"><i>arParams</i></span><i>['USER_ID']</i> будет передан код пользователя которого удалось
	 * авторизовать, а также массив с сообщением об ошибке <i><span class="syntax"><i>arParams</i><b></b></span>['RESULT_MESSAGE']</i>.
	 *
	 *
	 * @param array &$arParams Массив полей проверки имени входа и пароля:       <ul> <li> <b>USER_ID</b> - в
	 * случае, если авторизация прошла успешно, содержит код
	...
	/**
	 * Событие "OnAfterUserLogout" вызывается после <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/logout.php">завершения авторизации</a>
	 * пользователя.
	 *
	 *
	...
	/**
	 * Событие "OnAfterUserRegister" вызывается после попытки регистрации нового пользователя методом <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/register.php">CUser::Register</a>.
	 *
	 *
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?<br>// файл /bitrix/modules/my_module_id/include.php<br>class MyClass<br>{<br>    // создаем обработчик события "OnAfterUserRegister"<br>
	 *     function OnAfterUserRegisterHandler(&amp;$arFields)<br>    {<br>        // если регистрация успешна то<br>        if($arFields["USER_ID"]&gt;0)<br>
	 *           {<br>            // если текущий сайт - r1, то<br>            if(SITE_ID=="r1")<br>            {<br>                // зададим сообщение об
	 *     успешной регистрации на сайте r1<br>                $arFields["RESULT_MESSAGE"]["MESSAGE"] = "Вы успешно зарегистрировались на сайте \"Мой любимый
	 *     сайт 1\"";<br>            }<br>            elseif(SITE_ID=="r2")<br>            {<br>                // зададим сообщение об успешной регистрации на
	 *     сайте r2<br>                $arFields["RESULT_MESSAGE"]["MESSAGE"] = "Вы успешно зарегистрировались на сайте \"Мой любимый сайт 2\"";<br><br>
	 *         }<br>        }<br>        return $arFields;<br>    }<br>}<br>?&gt;&lt;?<br>// регистрируем обработчик события
	 *     "OnAfterUserRegister"<br>RegisterModuleDependences("main", "<b>OnAfterUserRegister</b>", "my_module_id", "MyClass",
	 *     "OnAfterUserRegisterHandler");<br>?&gt;
	 * </pre>
	 *
	 *
	 * <h4>See Also</h4>
	...
	/**
	 * Событие "OnAfterUserSimpleRegister" вызывается после попытки упрощённой регистрации нового пользователя методом <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/simpleregister.php">CUser::SimpleRegister</a>.
	 *
	 *
	...
	/**
	 * Событие <b>OnAfterUserUpdate</b> вызывается после попытки изменения свойств пользователя методом <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/update.php">CUser::Update</a>.
	 *
	 *
	...
	/**
	 * <p>Событие "OnBeforeChangeFile" вызывается при изменении файла методом <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cmain/savefilecontent.php">$APPLICATION-&gt;SaveFileContent</a>, перед его сохранением. Событие
	 * добавлено в версии 8.5.1 ядра. Контент в событие передается по ссылке.</p>
	 *
	 *
	...
	/**
	 * Событие <b>OnBeforeEventAdd</b> вызывается в момент добавления почтового события <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/general/mailevents.php">в таблицу b_event</a>. Как правило, задача обработчика данного события - изменить
	 * или добавить какое-либо значение, передаваемое в макросы почтового шаблона.   <p></p>
	 *
	 *
	...
	/**
	 * Событие "OnBeforeEventMessageDelete" вызывается перед <a href="http://dev.1c-bitrix.ru/api_help/main/reference/ceventmessage/delete.php">удалением
	 * почтового шаблона</a>. Как правило задачи обработчика данного события - разрешить или запретить удаление почтового шаблона.
	 *
	 *
	...
	/**
	 * <p>Событие вызывается в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cgroup/add.php">CGroup::Add</a> до добавления группы,  и может
	 * быть использовано для отмены добавления или переопределения некоторых полей.</p>
	 *
	 *
	 * @param array &$arFields Список полей (<a href="http://dev.1c-bitrix.ru/api_help/main/reference/cgroup/index.php">класс
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?<br>AddEventHandler("main", "OnBeforeGroupAdd", "MyOnBeforeGroupAdd");<br>function
	 *     MyOnBeforeGroupAdd(&amp;$arFields)<br>{<br>	if($arFields["DESCRIPTION"] == '')<br>        $arFields["DESCRIPTION"] = "Описание
	 *     группы";<br>}<br>?&gt;
	 * </pre>
	 *
	 *
	...
	/**
	 * Событие "OnBeforeGroupDelete" вызывается перед <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cgroup/delete.php">удалением группы
	 * пользователей</a>. Как правило задачи обработчика данного события - разрешить или запретить удаление группы пользователей.
	 *
	 *
	...
	/**
	 * <p>Событие вызывается в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cgroup/update.php">CGroup::Update</a> до изменения полей группы,
	 *  и может быть использовано для отмены изменения или переопределения некоторых полей.</p>
	 *
	 *
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?<br>AddEventHandler("main", "OnBeforeGroupUpdate", "MyOnBeforeGroupUpdate");<br>function MyOnBeforeGroupUpdate($ID,
	 *     &amp;$arFields)<br>{<br>	if($ID == 1)<br>        $arFields["DESCRIPTION"] = "Главная группа админов.";<br>}<br>?&gt;
	 * </pre>
	 *
	 *
	 * <h4>See Also</h4>
	...
	/**
	 * Событие "OnBeforeLanguageDelete" вызывается перед <a href="http://dev.1c-bitrix.ru/api_help/main/reference/clanguage/delete.php">удалением языка</a>.
	...
	/**
	 * Событие "OnBeforeSiteDelete" вызывается перед <a href="http://dev.1c-bitrix.ru/api_help/main/reference/csite/delete.php">удалением сайта</a>. Как
	 * правило задачи обработчика данного события - разрешить или запретить удаление сайта.
	 *
	 *
	...
	/**
	 * Событие вызывается в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/add.php">CUser::Add</a> до вставки нового пользователя, и
	...
	/**
	 * Событие "OnBeforeUserDelete" вызывается перед <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/delete.php">удалением пользователя</a>. Как
	 * правило задачи обработчика данного события - разрешить или запретить удаление пользователя.
	 *
	 *
	...
	/**
	 * Событие "OnBeforeUserLogin" вызывается в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/login.php">CUser::Login</a> до проверки
	 * имени входа <i><span class="syntax"><i>arParams</i></span>['LOGIN']</i> и пароля <i><span class="syntax"><i>arParams</i></span>['PASSWORD'] </i> и
	 * попытки авторизовать пользователя, и может быть использовано для прекращения процесса проверки или переопределения некоторых полей.
	 *
	 *
	...
	/**
	 * <p>Событие "OnBeforeUserLoginByHash" вызывается в методе <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/loginbyhash.php">CUser::LoginByHash()</a> до проверки имени входа <i><span
	 * class="syntax"><i>arParams</i><b></b></span>['LOGIN']</i>,  хеша от пароля <i><span class="syntax"><i>arParams</i><b></b></span>['HASH']</i> и попытки
	 * авторизовать пользователя.      </p>
	 *
	 *
	 * @param array &$arParams Массив полей для проверки имени входа и пароля:       <ul> <li> <b>LOGIN</b> -
	...
	/**
	 * <p>Вызывается перед завершением сеанса авторизации пользователя методом <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/logout.php">CUser::Logout</a> и может быть использовано для отмены завершения сеанса. </p>
	 *
	 *
	...
	/**
	 * Событие "OnBeforeUserSimpleRegister" вызывается до попытки упрощённой регистрации нового пользователя методом <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/simpleregister.php">CUser::SimpleRegister</a>  и может быть использовано для прекращения
	...
	/**
	 * Событие вызывается в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/update.php">CUser::Update</a> до изменения параметров
	 * пользователя, и может быть использовано для отмены изменения или переопределения некоторых полей.
	 *
	 *
	 ...
	 *     }
	 * }
	 * ?&gt;Смотрите также<li><a href="http://dev.1c-bitrix.ru/community/webdev/user/11948/blog/11321/">Как измененить пароль пользователя с подтверждением
	 *     старого</a></li>
	 * </pre>
	 *
	 *
	...
	/**
	 * Событие "OnChangePermissions" вызывается при изменении прав доступа  к файлу или папке методом <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cmain/setfileaccesspermission.php">$APPLICATION-&gt;SetFileAccessPermission</a>.
	 *
	 *
	...
	/**
	 * Событие "OnEventMessageDelete" вызывается во время <a href="http://dev.1c-bitrix.ru/api_help/main/reference/ceventmessage/delete.php">удаления почтового
	 * шаблона</a>. Как правило задачи обработчика данного события - очистить базу данных от записей связанных с удаляемым почтовым шаблоном.
	 *
	 *
	 * @param int $template_id ID удаляемого почтового шаблона.
	 *
	...
	/**
	 * Событие "OnExternalAuthList" вызывается для получения списка источников внешней авторизации<a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/clanguage/delete.php"></a> при вызове метода <a
	 * href="../cuser/getexternalauthlist.php.html">CUser::GetExternalAuthList</a>.
	 *
	 *
	 ...
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?
	 * AddEventHandler("main", "OnExternalAuthList", Array("__IPBAuth", "OnExternalAuthList"));<br>class __IPBAuth<br>{<br>  function OnExternalAuthList()<br>
	 *     {<br>     return Array( Array("ID"=&gt;"IPB", "NAME"=&gt;"Invision Power Board")<br>      );<br> }<br>}
	 * ?&gt;
	 * </pre>
	 *
	 *
	...
	/**
	 * <p>Событие "OnFileDelete" вызывается после удаления файла в методе <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cfile/delete.php">CFile::Delete</a>. Событие может использоваться для удаления производной от
	 * файла информации (созданных при загрузке картинки эскизов и т.п.).</p>
	 *
	 *
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;?<br>AddEventHandler("main", "OnFileDelete", "MyOnFileDelete");<br>function MyOnFileDelete($arFile)<br>{<br>	$fname =
	 *     $_SERVER["DOCUMENT_ROOT"]."/upload/resize/".$arFile["SUBDIR"]."/small_".$arFile["FILE_NAME"];<br>	if(file_exists($fname))<br>
	 *     unlink($fname);<br><br>}<br><br>?&gt;
	 * </pre>
	 *
	 *
	 * <h4>See Also</h4>
	...
	/**
	 * Событие "OnGroupDelete" вызывается в момент <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cgroup/delete.php">удаления группы
	 * пользователей</a>. Как правило задачи обработчика данного события - очистить базу данных от записей связанных с удаляемой группой пользователей.
	 *
	 *
	 * @param int $group_id ID удаляемой группы пользователей.
	 *
	...
	/**
	 * Событие "OnLanguageDelete" вызывается во время <a href="http://dev.1c-bitrix.ru/api_help/main/reference/clanguage/delete.php">удаления языка</a>. Как
	 * правило задачи обработчика данного события - очистить базу данных от записей связанных с удаляемым языком.
	 *
	 *
	...
	/**
	 * Событие "OnPanelCreate" вызывается в момент сбора данных для <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/reference/cmain/showpanel.php">построения</a> <a
	 * href="http://dev.1c-bitrix.ru/api_help/main/general/panel.php">панели управления</a> в публичной части сайта. Как правило задачи обработчика данного
	 * события - добавлять свои кнопки в панель управления сайтом.
	 *
	 *
	 * @return mixed
	 *
	...
	/**
	 * Событие "OnSendUserInfo" вызывается в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/senduserinfo.php">CUser::SendUserInfo</a> и
	 * предназначено для возможности переопределения параметров для <a href="http://dev.1c-bitrix.ru/api_help/main/general/mailevents.php">отправки почтового
	 * события</a> USER_INFO.
	 *
	 *
	 ...
	 * <h4>Example</h4>
	 * <pre bgcolor="#323232" style="padding:5px;">
	 * &lt;? <br>// файл /bitrix/php_interface/init.php <br>AddEventHandler("main", "OnSendUserInfo", "MyOnSendUserInfoHandler"); <br>function
	 *     MyOnSendUserInfoHandler(&amp;$arParams) <br>{ <br>   if(strlen($arParams['USER_FIELDS']['LAST_NAME'])&lt;=0) <br>
	 *     $arParams['FIELDS']['CUSTOM_NAME'] = $arParams['USER_FIELDS']['LAST_NAME']; <br>   else <br>       $arParams['FIELDS']['CUSTOM_NAME'] =
	 *     $arParams['USER_FIELDS']['LOGIN']; <br>   // теперь в шаблоне USER_INFO можно использовать макрос #CUSTOM_NAME# <br>} <br>? &gt;
	 * </pre>
	 *
	 *
	 * <h4>See Also</h4>
	...
	/**
	 * Событие "OnSiteDelete" вызывается во время <a href="http://dev.1c-bitrix.ru/api_help/main/reference/csite/delete.php">удаления сайта</a>. Как правило
	 * задачи обработчика данного события - очистить базу данных от записей связанных с удаляемым сайтом.
	 *
	 *
	...
	/**
	 * Событие "OnUserDelete" вызывается в момент <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/delete.php">удаления пользователя</a>. Как
	 * правило задачи обработчика данного события - очистить базу данных от записей связанных с удаляемым пользователем.
	 *
	 *
	 * @param int $user_id ID удаляемого пользователя.
	 *
	...
	/**
	 * <p>Событие <b>OnUserLoginExternal</b> предназначено для возможности проверки имени входа и пароля во внешнем источнике. Обработчики этого события
	 * вызываются в методе <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cuser/login.php">CUser::Login</a>, если ни один обработчик события <b>
	 * OnBegoreUserLogin</b> не вернул <i>false</i>,  перед стандартной проверкой имени входа <span class="syntax"><i>arParams</i></span><i>['LOGIN']</i>,
	 * пароля <span class="syntax"><i>arParams</i></span><i>['PASSWORD']</i> и попытки авторизовать пользователя.</p>
	 *
	 *
	 * @param array &$arParams Массив полей для проверки имени входа и пароля:       <ul> <li> <b>LOGIN</b> -
	 * Логин пользователя</li>         <li> <b>PASSWORD</b> - Пароль. Если параметр
