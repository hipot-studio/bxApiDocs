...
class CAllAgent
{
	 ...
	 * <p>
	 *     <img src="https://dev.1c-bitrix.ru/images/dev_full/technology/agents/agents_admin.png"><br>
	 * В стандартной конфигурации агентов, переведенных выполняться на cron - на хитах будут исполняться только периодические агенты (период выполнения - точно, в указанное время).</p>
	 *
	 *
	 * @param string $name  Строка PHP для запуска агента-функции.<br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param string $module = "" Идентификатор модуля. Необходим для подключения файлов модуля.<br>Необязательный. По умолчанию пустой.<br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param string $period = "N" <p>Если значение - "Y", то очередная дата запуска агента (<i>next_exec</i>) будет рассчитываться как:</p> <pre><i>next_exec</i> = <i>next_exec</i> + <i>interval</i></pre> Т.е. при очередном запуске, если прошло уже больше времени чем указано в параметре <i>interval</i>, агент сначала будет запускаться ровно столько раз сколько он должен был запуститься (т.е. столько раз сколько он "пропустил"), а затем, когда <i>next_exec</i> достигнет либо превысит текущую дату, он будет в дальнейшем запускаться с периодичностью указанной в параметре <i>interval</i>. Как правило, подобное используется в агентах которые должны гарантированно запуститься определённое количество раз. <p>Значение period = "Y" соответствует настройке агента в административной панели <strong>Периодичность выполнения</strong>: точно в указанное время.</p> <p>Если значение - "N", то очередная дата запуска агента (<i>next_exec</i>) будет рассчитываться как:</p> <pre><i>next_exec</i> = дата последнего запуска + <i>interval</i></pre>Т.е. агент после первого запуска будет в дальнейшем запускаться с периодичностью указанной в параметре <i>interval</i>. <p>Значение period = "N" соответствует настройке агента в административной панели <strong>Периодичность выполнения</strong>: через заданный интервал.</p> Параметр необязательный, по умолчанию - "N".<br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param int $interval = 86400 Интервал (в секундах), с какой периодичностью запускать агента.<br> Необязательный. По умолчанию - 86400 (1 сутки).<br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param string $datecheck = "" Дата первой проверки "не пора ли запустить агент" в формате текущего языка.<br> Необязательный. По умолчанию - текущее время.<br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param string $active = "Y" Активность агента (Y|N).<br> Необязательный. По умолчанию - "Y" (активен).<br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param string $next_exec = "" Дата первого запуска агента в формате текущего языка.<br>Необязательный. По умолчанию - текущее время. (С версии 3.1.4)<br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param int $sort = 100 Индекс сортировки позволяющий указать порядок запуска данного агента относительно других агентов для которых подошло время запуска.<br>Необязательный. По умолчанию - 100. (С версии 3.2.9)<br /><br /><hr /><br /><br />
	 * @param bool|int $user_id user<br /><br /><hr /><br /><br />
	 * @param bool $existError = true return error, if agent already exist<br /><br /><hr /><br /><br />
	 *
	 * @return mixed <p>При успешном выполнении, возвращает ID вновь добавленного агента, иначе - <i>false</i>. Если агент ничего не возвращает, он удаляется. Как правило он должен вернуть вызов самого себя.</p>
	 *
	 * @link https://dev.1c-bitrix.ru/api_help/main/reference/cagent/addagent.php
	 * @author phpDoc author - generator by hipot at 28.12.2025
	 */
