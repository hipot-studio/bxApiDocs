...
class utils
{
    ...
    public static function LoadWizardData($wizard)
    {
        ...
        if (defined('WIZARD_DEFAULT_TONLY') && WIZARD_DEFAULT_TONLY === true && !defined('WIZARD_DEFAULT_DONLY') && 'INSTALL' !== strtoupper($arWizardDescription['WIZARD_TYPE'])) {
            return false;
        }
        if (defined('WIZARD_DEFAULT_TONLY') && WIZARD_DEFAULT_TONLY === true && defined('WIZARD_DEFAULT_DONLY') && 'INSTALL' !== strtoupper($arWizardDescription['WIZARD_TYPE']) && 'INSTALL_ONCE' !== strtoupper($arWizardDescription['WIZARD_TYPE'])) {
            return false;
        }
        if ((!defined('WIZARD_DEFAULT_TONLY') || WIZARD_DEFAULT_TONLY !== true) && 'INSTALL' !== strtoupper($arWizardDescription['WIZARD_TYPE']) && 'INSTALL_ONCE' !== strtoupper($arWizardDescription['WIZARD_TYPE'])) {
            return false;
        }
    ...
    public static function VersionCompare($strCurver, $strMinver, $strMaxver = '0.0.0')
    {
        ...
        for ($i = 0; $i < 3; ++$i) {
            $curver[$i] = (isset($curver[$i]) ? (int) ($curver[$i]) : 0);
        }
        $minver = explode('.', $strMinver);
        for ($i = 0; $i < 3; ++$i) {
            $minver[$i] = (isset($minver[$i]) ? (int) ($minver[$i]) : 0);
        }
        $maxver = explode('.', $strMaxver);
        for ($i = 0; $i < 3; ++$i) {
            $maxver[$i] = (isset($maxver[$i]) ? (int) ($maxver[$i]) : 0);
        }
        ...
        if (($maxver[0] > 0 || $maxver[1] > 0 || $maxver[2] > 0)
            && (
                $curver[0] > $maxver[0]
    ...
    public static function Add2Log($sText, $sErrorCode = '')
    {
        ...
        if (file_exists($LOG_FILE)) {
            ...
            if ($log_size > $MAX_LOG_SIZE) {
                ...
                if (!($fp1 = @fopen($LOG_FILE_TMP, 'w'))) {
                    ignore_user_abort($old_abort_status);

                    return false;
                }

                $iSeekLen = (int) ($log_size - $MAX_LOG_SIZE / 2.0);
                fseek($fp, $iSeekLen);
    ...
    public static function IsUTF8Support()
    {
        return
            ...
            && 'UTF-8' === strtoupper(ini_get('default_charset'));
    }
    ...
    public static function EncodeFile($filePath, $charsetFrom)
    {
        ...
        if ('php' !== $extension && 'sql' !== $extension && 'js' !== $extension && 'csv' !== $extension && 'snp' !== $extension && 'html' !== $extension && 'xml' !== $extension) {
            return;
        }
        ...
        if ('xml' === $extension) {
            if ('<?xml' === substr($fileContent, 0, 5) && !str_contains(substr($fileContent, 0, 100), 'encoding')) {
        ...
        while (($file = readdir($handle)) !== false) {
            ...
            if (is_dir($filePath)) {
                ...
                if (
                    ...
                    || str_contains($filePath, '/bitrix/modules/main/lang/'.LANGUAGE_ID.'/install.php')
                ) {
                    continue;
                }
                ...
                if ($encodeALL) {
                    ...
                    } elseif ('ru' === $matches[1] || 'ua' === $matches[1]) {
    ...
    {
        $bCgi = (false !== stristr(PHP_SAPI, 'cgi'));
    ...
    public static function LocalRedirect($url)
    {
        ...
        if (
            ...
            || 'https://' === strtolower(substr($url, 0, 8))
            || 'ftp://' === strtolower(substr($url, 0, 6))) {
            header("Request-URI: {$url}");
            header("Content-Location: {$url}");
            header("Location: {$url}");
        } else {
            if ('80' !== $SERVER_PORT && 443 !== $SERVER_PORT && $SERVER_PORT > 0 && strpos($HTTP_HOST, ':'.$SERVER_PORT) <= 0) {
                $HTTP_HOST .= ':'.$SERVER_PORT;
            }

            $protocol = (443 === $_SERVER['SERVER_PORT'] || 'on' === strtolower($_SERVER['HTTPS']) ? 'https' : 'http');
    ...
    public static function CheckSession()
    {
        ...
        return  isset($_SESSION['session_check']) && 'Y' === $_SESSION['session_check'];
    }
    ...
    public static function GetWizardsSettings()
    {
        ...
        $configFileContent = str_replace(['<config>', '</config>'], '', $configFileContent);

        preg_match_all('~<([a-zA-Z0-9]+[^>]*)>(.*?)</\\1>~s', $configFileContent, $arMatch);

        if (isset($arMatch[1], $arMatch[2]) && is_array($arMatch[1]) && is_array($arMatch[2])) {
            for ($i = 0, $length = count($arMatch[1]); $i < $length; ++$i) {
                $tagContent = str_replace(['&amp;', '&gt;', '&lt;', '&apos;', '&quot;'], ['&', '>', '<', "'", '"'], $arMatch[2][$i]);
    ...
    public static function GetRegistrationKey($lic_key_user_name, $lic_key_user_surname, $lic_key_email, $DBType)
    {
        ...
        if ($fp) {
            ...
            fwrite($fp, "Host: {$host}\r\n");
            fwrite($fp, 'Content-type: application/x-www-form-urlencoded; charset="'.$charset."\"\r\n");
            fwrite($fp, "User-Agent: bitrixKeyReq\r\n");
            fwrite($fp, 'Content-length: '.(function_exists('mb_strlen') ? mb_strlen($query, 'latin1') : strlen($query))."\r\n");
