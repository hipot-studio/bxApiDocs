...
class CreatedByFieldAssembler extends UserFieldAssembler
{
    protected function prepareColumn($value): string
    {
        ...
        $isFiltered = isset($ormFilter['CREATED_BY_ID']) && \in_array((int) $value['CREATED_BY_ID'], $ormFilter['CREATED_BY_ID'], true);

        $createdById = (int) $value['CREATED_BY_ID'];
        $type = $value['TYPE'];

        if (SupersetDashboardTable::DASHBOARD_TYPE_MARKET === $type) {
        ...
        if (SupersetDashboardTable::DASHBOARD_TYPE_SYSTEM === $type) {
