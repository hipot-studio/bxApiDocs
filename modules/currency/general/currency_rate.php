...
class currency_rate
{
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if ('UPDATE' === $ACTION && 0 >= (int) $ID) {
            $arMsg[] = ['id' => 'ID', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_ID_BAD')];
        }

        if (!isset($arFields['CURRENCY'])) {
            $arMsg[] = ['id' => 'CURRENCY', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_CURRENCY_ABSENT')];
        } else {
            $arFields['CURRENCY'] = mb_substr($arFields['CURRENCY'], 0, 3);
        }

        if (empty($arFields['DATE_RATE'])) {
            $arMsg[] = ['id' => 'DATE_RATE', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_DATE_ABSENT')];
        } elseif (!$DB->IsDate($arFields['DATE_RATE'])) {
            $arMsg[] = ['id' => 'DATE_RATE', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_DATE_FORMAT_BAD')];
        }

        if (is_set($arFields, 'RATE_CNT') || 'ADD' === $ACTION) {
            if (!isset($arFields['RATE_CNT'])) {
                $arMsg[] = ['id' => 'RATE_CNT', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_RATE_CNT_ABSENT')];
            } else {
                $arFields['RATE_CNT'] = (int) $arFields['RATE_CNT'];
                if ($arFields['RATE_CNT'] <= 0) {
                    $arMsg[] = ['id' => 'RATE_CNT', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_RATE_CNT_BAD')];
                }
            }
        }
        if (is_set($arFields['RATE']) || 'ADD' === $ACTION) {
            if (!isset($arFields['RATE'])) {
                $arMsg[] = ['id' => 'RATE', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_RATE_ABSENT')];
            } else {
                $arFields['RATE'] = (float) $arFields['RATE'];
                if (!($arFields['RATE'] > 0)) {
                    $arMsg[] = ['id' => 'RATE', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_RATE_BAD')];
                }
            }
        }
        if ('ADD' === $ACTION) {
            if ($arFields['CURRENCY'] === Currency\CurrencyManager::getBaseCurrency()) {
                $arMsg[] = ['id' => 'CURRENCY', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_FOR_BASE_CURRENCY')];
            } else {
            ...
            if ($arFields['CURRENCY'] === $arFields['BASE_CURRENCY']) {
                $arMsg[] = ['id' => 'CURRENCY', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_FOR_SELF_CURRENCY')];
            }
        }
    ...
    public static function Add($arFields)
    {
        ...
        if ($db_result->Fetch()) {
            $arMsg[] = ['id' => 'DATE_RATE', 'text' => GetMessage('ERROR_ADD_REC2')];
    ...
    public static function Delete($ID)
    {
        ...
        if (!is_array($arFields)) {
            $arMsg = ['id' => 'ID', 'text' => GetMessage('BT_MOD_CURR_ERR_RATE_CANT_DELETE_ABSENT_ID')];
            $e = new CAdminException($arMsg);
            $APPLICATION->ThrowException($e);
    ...
    public static function GetList($by = 'date', $order = 'asc', $arFilter = [])
    {
        ...
        if ('curr' === strtolower($by)) {
            $strSqlOrder = ' ORDER BY C.CURRENCY ';
        } elseif ('rate' === strtolower($by)) {
    ...
    public static function GetConvertFactorEx($curFrom, $curTo, $valDate = '')
    {
        ...
        if (0 === $curFromRate || 0 === $curToRateCnt || 0 === $curToRate || 0 === $curFromRateCnt) {
            return 0;
        }
