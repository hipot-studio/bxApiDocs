...
class CAllIBlockElement
{
	...
	/**
	 * <p>Метод сохраняет значения всех свойств элемента информационного блока. В отличие от <a class="link" href="https://dev.1c-bitrix.ru/api_help/iblock/classes/ciblockelement/setpropertyvalues.php">SetPropertyValues</a> может не содержать полный набор значений. Значения свойств, неуказанных в массиве PROPERTY_VALUES, будут сохранены. Этот метод более экономен в количестве запросов к БД. Метод статический.</p><p> Метод возвращает <i>Null</i>.</p><p>Если необходимо сохранить пустое значение для множественного свойства, то используйте значение <i>false</i>, так как просто пустой массив не сохранится.</p>
	 *
	 *
	 *
	 *
	 * @param int $ELEMENT_ID  Код элемента, значения свойств которого необходимо установить.<br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param int $IBLOCK_ID  Код информационного блока. Может быть не указан. В этом случае будет прочитан из базы данных по коду элемента. <br><br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param array $PROPERTY_VALUES  Массив значений свойств, в котором коду свойства ставится в соответствие значение свойства. <br> Должен быть вида Array("код свойства1"=&gt;"значения свойства1", ....), где "код свойства" - числовой или символьный код свойства, "значения свойства" - одно или массив всех значений свойства (множественное).<br><br> <div class="note"> <b>Примечания</b>: <ul> <li>При добавлении значений свойств типа "Файл" поле DESCRIPTION обязательно.</li> <li>Если передаётся массив "свойство"=&gt;"значение", то в качестве значения свойств типа "справочник" нужно указывать внешний код элемента справочника.</li> <li>Для свойства типа <b>Список</b> следует передавать идентификатор значения свойства, а не значение.</li> <li>для свойства типа <strong>Привязка к файлу на сервере</strong>, нужно передавать путь к файлу от корня сайта.</li> </ul> </div><br /><br /><hr /><br /><br />
	 *
	 *
	 *
	 * @param array{'NewElement':bool, 'DoNotValidateLists':bool} $FLAGS = array() Необязательный параметр предоставляет информацию для оптимизации выполнения. Может содержать следующие ключи: <br> <ul> <li>NewElement - можно указать если заведомо известно, что значений свойств у данного элемента нет. Экономит один запрос к БД.</li> <li>DoNotValidateLists - для свойств типа "список" отключает проверку наличия значений в метаданных свойства. <br> </li> </ul>
	 *
	 *
	 *
	 * @return null 
	 ...
	 * @author phpDoc author - generator by hipot at 16.02.2025
	 */
