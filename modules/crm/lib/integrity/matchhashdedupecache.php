...
class MatchHashDedupeCache
{
	...
	protected function createTable(Query $baseQuery): Result
	{
		...
		DbHelper::queryByDbType(
			...
			. " (MATCH_HASH VARCHAR(32) NOT NULL, QTY INT(1) UNSIGNED NOT NULL)",
			"CREATE TABLE IF NOT EXISTS $tableName (MATCH_HASH varchar(32) NOT NULL, QTY int8 NOT NULL)"
		);

		$connection->queryExecute("INSERT INTO $tableName $subQuery");

		$this->isExists = null;
		if ($this->isExists())
		{
			$this->currentTableName = $tableName;
			$connection->createPrimaryIndex($tableName, 'MATCH_HASH');
		}
