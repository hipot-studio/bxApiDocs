...
class blog_post_category
{
    ...
    {
        if ((is_set($arFields, 'POST_ID') || 'ADD' === $ACTION) && '' === $arFields['POST_ID']) {
        ...
        if ((is_set($arFields, 'CATEGORY_ID') || 'ADD' === $ACTION) && (int) $arFields['CATEGORY_ID'] <= 0) {
            $GLOBALS['APPLICATION']->ThrowException(GetMessage('BLG_GCT_EMPTY_CATEGORY_ID'), 'EMPTY_CATEGORY_ID');

            return false;
        }
        if (is_set($arFields, 'CATEGORY_ID')) {
