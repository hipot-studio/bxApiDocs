...
class ratings_components
{
    ...
    public static function OnGetRatingConfigs()
    {
        ...
        $arConfigs['COMPONENT']['USER']['RATING'][] = [
            'ID' => 'ACTIVITY',
            'REFRESH_TIME' => '7200',
            'CLASS' => 'CRatingsComponentsBlog',
    ...
    {
        if ('BLOG_POST' === $arParams['ENTITY_TYPE_ID']) {
            $arPost = CBlogPost::GetByID((int) $arParams['ENTITY_ID']);

            return $arPost['AUTHOR_ID'];
        }
        if ('BLOG_COMMENT' === $arParams['ENTITY_TYPE_ID']) {
            $arComment = CBlogComment::GetByID((int) $arParams['ENTITY_ID']);
