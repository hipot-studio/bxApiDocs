...
class blog_post_category extends CAllBlogPostCategory
{
    // ADD, UPDATE, DELETE
    ...
    public static function GetList($arOrder = ['ID' => 'DESC'], $arFilter = [], $arGroupBy = false, $arNavStartParams = false, $arSelectFields = [])
    {
        ...
        if (count($arSelectFields) <= 0) {
            $arSelectFields = ['ID', 'BLOG_ID', 'POST_ID', 'CATEGORY_ID', 'NAME'];
        }
        ...
        $arFields = [
            ...
            'CATEGORY_ID' => ['FIELD' => 'C.CATEGORY_ID', 'TYPE' => 'int'],
            'NAME' => ['FIELD' => 'CC.NAME', 'TYPE' => 'string', 'FROM' => 'LEFT JOIN b_blog_category CC ON (CC.ID = C.CATEGORY_ID)'],
            'POST_PUBLISH_STATUS' => ['FIELD' => 'P.PUBLISH_STATUS', 'TYPE' => 'string', 'FROM' => 'INNER JOIN b_blog_post P ON (P.ID = C.POST_ID)'],
        ];
        // <-- FIELDS
