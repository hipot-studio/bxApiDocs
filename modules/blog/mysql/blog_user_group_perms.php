...
class blog_user_group_perms extends CAllBlogUserGroupPerms
{
    // ADD, UPDATE, DELETE
    ...
    public static function GetList($arOrder = ['ID' => 'DESC'], $arFilter = [], $arGroupBy = false, $arNavStartParams = false, $arSelectFields = [])
    {
        ...
        if (count($arSelectFields) <= 0) {
            $arSelectFields = ['ID', 'BLOG_ID', 'USER_GROUP_ID', 'PERMS_TYPE', 'POST_ID', 'PERMS', 'AUTOSET'];
        }

        // FIELDS -->
        $arFields = [
            'ID' => ['FIELD' => 'GP.ID', 'TYPE' => 'int'],
            'BLOG_ID' => ['FIELD' => 'GP.BLOG_ID', 'TYPE' => 'int'],
            'USER_GROUP_ID' => ['FIELD' => 'GP.USER_GROUP_ID', 'TYPE' => 'int'],
            'PERMS_TYPE' => ['FIELD' => 'GP.PERMS_TYPE', 'TYPE' => 'char'],
            'POST_ID' => ['FIELD' => 'GP.POST_ID', 'TYPE' => 'int'],
            'PERMS' => ['FIELD' => 'GP.PERMS', 'TYPE' => 'char'],
            'AUTOSET' => ['FIELD' => 'GP.AUTOSET', 'TYPE' => 'char'],
        ];
        // <-- FIELDS
