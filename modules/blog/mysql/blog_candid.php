...
class blog_candid extends CAllBlogCandidate
{
    // ADD, UPDATE, DELETE
    ...
    public static function GetList($arOrder = ['ID' => 'DESC'], $arFilter = [], $arGroupBy = false, $arNavStartParams = false, $arSelectFields = [])
    {
        ...
        if (count($arSelectFields) <= 0) {
            $arSelectFields = ['ID', 'BLOG_ID', 'USER_ID'];
        }
        ...
        $arFields = [
            ...
            'USER_ID' => ['FIELD' => 'C.USER_ID', 'TYPE' => 'int'],

            'USER_LOGIN' => ['FIELD' => 'U.LOGIN', 'TYPE' => 'string', 'FROM' => 'INNER JOIN b_user U ON (C.USER_ID = U.ID)'],
            'USER_NAME' => ['FIELD' => 'U.NAME', 'TYPE' => 'string', 'FROM' => 'INNER JOIN b_user U ON (C.USER_ID = U.ID)'],
            'USER_LAST_NAME' => ['FIELD' => 'U.LAST_NAME', 'TYPE' => 'string', 'FROM' => 'INNER JOIN b_user U ON (C.USER_ID = U.ID)'],
            'USER_SECOND_NAME' => ['FIELD' => 'U.SECOND_NAME', 'TYPE' => 'string', 'FROM' => 'INNER JOIN b_user U ON (C.USER_ID = U.ID)'],
            'USER_EMAIL' => ['FIELD' => 'U.EMAIL', 'TYPE' => 'string', 'FROM' => 'INNER JOIN b_user U ON (C.USER_ID = U.ID)'],
            'USER' => ['FIELD' => 'U.LOGIN,U.NAME,U.LAST_NAME,U.SECOND_NAME,U.EMAIL,U.ID', 'WHERE_ONLY' => 'Y', 'TYPE' => 'string', 'FROM' => 'INNER JOIN b_user U ON (C.USER_ID = U.ID)'],

            'BLOG_USER_ID' => ['FIELD' => 'BU.ID', 'TYPE' => 'int', 'FROM' => 'LEFT JOIN b_blog_user BU ON (C.USER_ID = BU.USER_ID)'],
            'BLOG_USER_ALIAS' => ['FIELD' => 'BU.ALIAS', 'TYPE' => 'string', 'FROM' => 'LEFT JOIN b_blog_user BU ON (C.USER_ID = BU.USER_ID)'],
        ];
        // <-- FIELDS
