...
class ratings_components extends CAllRatingsComponentsBlog
{
    public static function CalcPost($arConfigs)
    {
        ...
        $strSql = "DELETE FROM b_rating_component_results WHERE RATING_ID = '".(int) $arConfigs['RATING_ID']."' AND COMPLEX_NAME = '".$DB->ForSql($arConfigs['COMPLEX_NAME'])."'";
        ...
        $strSql = "INSERT INTO b_rating_component_results (RATING_ID, MODULE_ID, RATING_TYPE, NAME, COMPLEX_NAME, ENTITY_ID, ENTITY_TYPE_ID, CURRENT_VALUE)
					SELECT
						'".(int) $arConfigs['RATING_ID']."'  RATING_ID,
						...
						SUM(RVE.VALUE)*".(float) $arConfigs['CONFIG']['COEFFICIENT']."  CURRENT_VALUE
					FROM
						b_rating_voting RV LEFT JOIN b_blog_post FT ON RV.ENTITY_ID = FT.ID,
						b_rating_vote RVE
					WHERE
						RV.ENTITY_TYPE_ID = 'BLOG_POST' AND FT.AUTHOR_ID > 0
					AND RVE.RATING_VOTING_ID = RV.ID".((int) $arConfigs['CONFIG']['LIMIT'] > 0 ? ' AND RVE.CREATED > DATE_SUB(NOW(), INTERVAL '.(int) $arConfigs['CONFIG']['LIMIT'].' DAY)' : '').'
					GROUP BY AUTHOR_ID';
        $res = $DB->Query($strSql, false, $err_mess.__LINE__);
    ...
    public static function CalcActivity($arConfigs)
    {
        ...
        $res = $DB->Query($strSql, false, $err_mess.__LINE__);
        $sqlAllPost = '';
        if (isset($arConfigs['CONFIG']['ALL_POST_COEF']) && 0 !== $arConfigs['CONFIG']['ALL_POST_COEF']) {
            $sqlAllPost = '
				SELECT
					AUTHOR_ID as ENTITY_ID,
					COUNT(*)*'.(float) $arConfigs['CONFIG']['ALL_POST_COEF']." as CURRENT_VALUE
				FROM b_blog_post
        ...
        $sqlAllComment = '';
        if (isset($arConfigs['CONFIG']['ALL_COMMENT_COEF']) && 0 !== $arConfigs['CONFIG']['ALL_COMMENT_COEF']) {
            $sqlAllComment = '
				SELECT
					AUTHOR_ID as ENTITY_ID,
					COUNT(*)*'.(float) $arConfigs['CONFIG']['ALL_COMMENT_COEF']." as CURRENT_VALUE
				FROM b_blog_comment
        ...
        $strSql = "INSERT INTO b_rating_component_results (RATING_ID, MODULE_ID, RATING_TYPE, NAME, COMPLEX_NAME, ENTITY_ID, ENTITY_TYPE_ID, CURRENT_VALUE)
			SELECT
				'".(int) $arConfigs['RATING_ID']."' as RATING_ID,
			...
			FROM (
				...
				SELECT
					AUTHOR_ID as ENTITY_ID,
					SUM(IF(TO_DAYS(DATE_PUBLISH) > TO_DAYS(NOW())-1, 1, 0))*'.(float) $arConfigs['CONFIG']['TODAY_POST_COEF'].' +
					SUM(IF(TO_DAYS(DATE_PUBLISH) > TO_DAYS(NOW())-7, 1, 0))*'.(float) $arConfigs['CONFIG']['WEEK_POST_COEF'].'+
				...
				SELECT
					AUTHOR_ID as ENTITY_ID,
					SUM(IF(TO_DAYS(DATE_CREATE) > TO_DAYS(NOW())-1, 1, 0))*'.(float) $arConfigs['CONFIG']['TODAY_COMMENT_COEF'].' +
					SUM(IF(TO_DAYS(DATE_CREATE) > TO_DAYS(NOW())-7, 1, 0))*'.(float) $arConfigs['CONFIG']['WEEK_COMMENT_COEF'].' +
