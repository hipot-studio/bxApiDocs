...
class ImComponentConferenceEdit extends CBitrixComponent
{
    ...
    public function executeComponent()
    {
        if (!Loader::includeModule('im')) {
            $this->errorCollection[] = new Error('IM module is not installed');
        }
    ...
    protected function prepareResult(): bool
    {
        ...
        if ($editMode) {
            ...
            if (!$this->conference) {
                $this->errorCollection[] = new Error(Loc::getMessage('IM_CONFERENCE_EDIT_CLASS_ERROR_CONFERENCE_NOT_FOUND'));

                return false;
            }

            if (!$this->conference->canUserEdit(CurrentUser::get()->getId())) {
                $this->errorCollection[] = new Error(Loc::getMessage('IM_CONFERENCE_EDIT_CLASS_ERROR_CONFERENCE_CANT_EDIT'));

                return false;
            }

            if (!Chat::isUserInChat($this->conference->getChatId())) {
                $this->errorCollection[] = new Error(Loc::getMessage('IM_CONFERENCE_EDIT_CLASS_ERROR_CONFERENCE_ACCESS_DENIED'));
            ...
            if ($this->conference->isBroadcast()) {
                ...
                $this->arResult['PRESENTERS'] = array_map(static function ($user) {
                    return [
                        'id' => $user['id'],
                        'title' => $user['name'],
                        'avatar' => $user['avatar'],
