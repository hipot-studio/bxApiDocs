...
class CAllSearch extends CDBResult
{
	...
	/**
	 * <p>Метод инициализирует выборку данных. Нестатический метод.</p><p>Перед выполнением поисковых запросов вызывается событие <a class="link" href="https://dev.1c-bitrix.ru/api_help/search/events/onsearch.php">OnSearch</a>.</p>
	 *
	 * @param array $arParams  Массив, содержащий условия поиска в виде наборов "название поля"=&gt;"значение". <br> <br> Название поля может принимать значение: <ul> <li> <b>QUERY</b> - Строка запроса. Обязательный элемент. Должна быть сформирована в соответствии с правилами языка запросов.</li> <li> <b>TAGS</b> - Список тегов. В результате поиска будут возвращены все элементы имеющие данные теги.</li> <li> <b>SITE_ID</b> - Сайт, в информации которого производится поиск. Не обязательный параметр, по умолчанию равен текущему.</li> <li> <b>MODULE_ID</b> - Код модуля, данными которого ограничивается поиск. Если этот параметр равен false, то поиск производится по всем модулям. Не обязательный параметр, по умолчанию равен false.</li> <li> <b>ITEM_ID</b> - Код проиндексированного элемента. Используется для ограничения области поиска по коду элемента. Если параметр имеет значение false, то ограничение не производится. Не обязательный параметр, по умолчанию равен false.</li> <li> <b>PARAM1</b> - Первый параметр элемента или массив первых параметров. Используется для ограничения области поиска по произвольному параметру. Если параметр имеет значение false, то ограничение не производится. Не обязательный параметр, по умолчанию равен false.</li> <li> <b>PARAM2</b> - Второй параметр элемента или массив вторых параметров. Используется для ограничения области поиска по произвольному параметру. Если параметр имеет значение false, то ограничение не производится. Не обязательный параметр, по умолчанию равен false.</li> <li> <b>URL</b> - маска адреса относительно корня сайта, по которому доступен данный элемент или массив масок адресов;</li> <li> <b>DATE_CHANGE</b> - время изменения элемента в формате сайта (включает время);</li> <li> <b>CHECK_DATES</b> - если задан и равен Y, то найдены будут только активные элементы;</li> </ul><br /><br /><hr /><br /><br />
	 * @param array $aSort = array() Массив, содержащий признак сортировки в виде наборов "название поля"=&gt;"направление". <br> <br> Название поля может принимать значение: <ul> <li> <b>ID</b> - идентификатор в поисковом индексе;</li> <li> <b>MODULE_ID</b> - идентификатор модуля;</li> <li> <b>ITEM_ID</b> - идентификатор элемента поискового индекса (например для форума это идентификатор сообщения);</li> <li> <b>TITLE</b> - заголовок;</li> <li> <b>PARAM1</b> - Первый параметр элемента или массив первых параметров. Используется для ограничения области поиска по произвольному параметру. Если параметр имеет значение false, то ограничение не производится. Не обязательный параметр, по умолчанию равен false.</li> <li> <b>PARAM2</b> - Второй параметр элемента или массив вторых параметров. Используется для ограничения области поиска по произвольному параметру. Если параметр имеет значение false, то ограничение не производится. Не обязательный параметр, по умолчанию равен false.</li> <li> <b>DATE_FROM</b> - дата начала активности элемента;</li> <li> <b>DATE_TO</b> - дата окончания активности элемента;</li> <li> <b>RANK</b> - вычисленное значение релевантности;</li> <li> <b>TITLE_RANK</b> - количество вхождений подстрок запроса в заголовок. Позволяет повысить значимость заголовка в результатах поиска;</li> <li> <b>CUSTOM_RANK</b> - заданное значение релевантности;</li> <li> <b>DATE_CHANGE</b> - время изменения элемента;</li> </ul> Направление сортировки может принимать значение: <ul> <li> <b>ASC</b> - по возрастанию;</li> <li> <b>DESC</b> - по убыванию.</li> </ul> Пример (именно это значение считается по умолчанию): <pre class="syntax">array("CUSTOM_RANK"=&gt;"DESC", "RANK"=&gt;"DESC", "DATE_CHANGE"=&gt;"DESC")</pre> В случае когда параметр bTagsCloud равен true, допустимыми полями являются: <ul> <li> <b>DATE_CHANGE</b> - время изменения элемента;</li> <li> <b>NAME</b> - значение тег;</li> <li> <b>CNT</b> - частота тега;</li> </ul> (С версии 4.0.10)<br /><br /><hr /><br /><br />
	 * @param array $aParamsEx = array() Массив массивов, содержащий дополнительные условия поиска в виде наборов "название поля"=&gt;"значение". Эти условия будут объединены по условию ИЛИ. Смотри описание параметра arParams. <br> <br> Так же может содержать элемент: <ul> <li> <b>STEMMING</b> - Если равно true, то при поиске будет учитываться морфология. Если false, то не будет. Если не задано, то будет использоваться настройка модуля;</li> </ul> Пример сложного условия: <pre class="syntax">array( array( "MODULE_ID" =&gt; "forum", "PARAM1" =&gt; array(1, 2), ), array( "MODULE_ID" =&gt; "iblock", "PARAM2" =&gt; 3, ), )</pre> В данном случае поиск будет осуществляться в форумах с идентификаторами 1 и 2, а также в инфоблоке с идентификатором 3. (С версии 4.0.10)<br /><br /><hr /><br /><br />
	 * @param bool $bTagsCloud = false Признак построения облака тегов. Если задан и равен true, то будет построено облако тегов. (С версии 6.5.0)
	 *
	 * @return void
	 * <code>
	 *     //combination ($MODULE_ID, $PARAM1, $PARAM2, $PARAM3) is used to narrow search
	 *     //holds recordset with search results
	 * </code>
	 *
	 * @link https://dev.1c-bitrix.ru/api_help/search/classes/csearch/search.php
	 * @author phpDoc author - generator by hipot at 30.04.2025
	 */
