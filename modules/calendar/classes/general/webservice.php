...
class webservice extends IWebService
{
    public $arStatusValues = [
        'free' => 0, 'quest' => 1, 'busy' => 2, 'absent' => 3,
    ];

    public $arPriorityValues = [
    ...
    public static function __makeTS($datetime = null)
    {
        ...
        if ((int) substr($datetime, 0, 4) >= 2_037) {
    ...
    public function __getRow($event, $listName, &$last_change)
    {
        ...
        $bAllDay = 'N' !== $event['DT_SKIP_TIME'] ? 1 : 0;
        ...
        */
        ...
        $obRow->setAttribute('ows_MetaInfo_BusyStatus', null === $status ? -1 : $status);
        ...
        if ($bRecurrent) {
            ...
            $tz_data .= '<standardBias>'.(-(int) (($TZBias - (date('I') ? 3_600 : 0)) / 60)).'</standardBias>';
            $tz_data .= '<additionalDaylightBias>-60</additionalDaylightBias>';

            $bUseTransition = 'Y' === COption::GetOptionString('intranet', 'tz_transition', 'Y');
            ...
            if ('2038' === date('Y', $ts_finish) || '2037' === date('Y', $ts_finish)) {
    ...
    public function GetList($listName)
    {
        ...
        $arSections = CCalendarSect::GetList(
            [
                'arFilter' => ['XML_ID' => strtolower($listName_original)],
            ]
        );
    ...
    public function GetListItemChanges($listName, $viewFields = '', $since = '', $contains = '')
    {
        ...
        return ['GetListItemChangesResult' => $res['GetListItemChangesSinceTokenResult']];
    }
    ...
    public function GetListItemChangesSinceToken($listName, $viewFields = '', $query = '', $rowLimit = 0, $changeToken = '')
    {
        ...
        $arSections = CCalendarSect::GetList(['arFilter' => ['XML_ID' => $listName_original]]);
        if (!$arSections || !is_array($arSections[0])) {
            return new CSoapFault(
                'List not found',
                'List with '.$listName.' GUID not found!'
        ...
        $fetchMeetings = 'user' === $arSection['CAL_TYPE'] && CCalendar::GetMeetingSection($arSection['OWNER_ID']) === $arSection['ID'];
        ...
        foreach ($arEvents as $event) {
            if ('N' !== $event['DELETED'] || ($event['IS_MEETING'] && 'N' === $event['MEETING_STATUS'])) {
        ...
        return ['GetListItemChangesSinceTokenResult' => $data];
    }
    ...
    public function UpdateListItems($listName, $updates)
    {
        ...
        $bGroup = 'group' === $arSection['CAL_TYPE'];
        ...
        if ($bGroup) {
            ...
            if ('Y' === $arGroupTmp['CLOSED']) {
                if ('Y' !== COption::GetOptionString('socialnetwork', 'work_with_closed_groups', 'N')) {
        ...
        foreach ($arChanges as $obMethod) {
            $arData = ['_command' => $obMethod->getAttribute('Cmd')];
            ...
            if ('Delete' === $arData['_command']) {
                ...
                */
            ...
            } elseif ('New' === $arData['_command'] || 'Update' === $arData['_command']) {
                ...
                if (isset($arData['EventDate'])) {
                    ...
                    $toTsUTC = $this->__makeTS($arData['EndDate']) + ($arData['fAllDayEvent'] ? -86_340 : 0);
                    $TZBias = (int) date('Z', $fromTsUTC);
                }

                $arData['EventType'] = (int) $arData['EventType'];

                if (2 === $arData['EventType']) {
                    $arData['EventType'] = 0;
                }
                ...
                $id = 'New' === $arData['_command'] ? 0 : (int) ($arData['ID']);
                if ($arData['RecurrenceData']) {
                    ...
                    switch ($obNode->name) {
                        case 'daily':
                            ...
                            if ('TRUE' === $obNode->getAttribute('weekday')) {
                    ...
                    if (0 === $arData['DT_LENGTH'] && isset($arData['RRULE']['FREQ'])) {
                        $arData['DT_LENGTH'] = 86_400;
                    }

                    $obWhile = $obRecurRule->children[2];
                    if ('repeatForever' === $obWhile->name) {
                        $arData['RRULE']['UNTIL'] = MakeTimeStamp('');
                    } elseif ('windowEnd' === $obWhile->name) {
                        $toTsUTC = $this->__makeTS($obWhile->textContent());
                        $arData['RRULE']['UNTIL'] = ConvertTimeStamp($toTsUTC, 'FULL');
                    }
                } elseif (-1 === $arData['fRecurrence'] && $id > 0) {
                    $arData['RRULE'] = -1;
                }
                ...
                $arFields = [
                    ...
                    'OWNER_ID' => $ownerId,
                    'CREATED_BY' => $userId,
                    'DATE_FROM' => CCalendar::Date($fromTs, 'N' === $skipTime),
                    'DATE_TO' => CCalendar::Date($toTs, 'N' === $skipTime),
        ...
        $wsdesc->classes = [
            'CCalendarWebService' => [
                'GetList' => [
                    ...
                    'name' => 'GetList',
                    'input' => [
                        'listName' => ['varType' => 'string'],
                    ],
                    'output' => [
                        'GetListResult' => ['varType' => 'any'],
                    ],
                    'httpauth' => 'Y',
                ],
                'GetListItemChanges' => [
                    ...
                    'input' => [
                        'listName' => ['varType' => 'string'],
                        'viewFields' => ['varType' => 'any', 'strict' => 'no'],
                        'since' => ['varType' => 'string', 'strict' => 'no'],
                    ],
                    'output' => [
                        'GetListItemChangesResult' => ['varType' => 'any'],
                    ],
                    'httpauth' => 'Y',
                ],
                'GetListItemChangesSinceToken' => [
                    ...
                    'input' => [
                        'listName' => ['varType' => 'string'],
                        'viewFields' => ['varType' => 'any', 'strict' => 'no'],
                        'query' => ['varType' => 'any', 'strict' => 'no'],
                        'rowLimit' => ['varType' => 'string', 'strict' => 'no'],
                        'changeToken' => ['varType' => 'string', 'strict' => 'no'],
                    ],
                    'output' => [
                        'GetListItemChangesSinceTokenResult' => ['varType' => 'any'],
                    ],
                    'httpauth' => 'Y',
                ],
                'UpdateListItems' => [
                    ...
                    'input' => [
                        'listName' => ['varType' => 'string'],
                        'updates' => ['varType' => 'any', 'strict' => 'no'],
                    ],
                    'output' => [
                        'UpdateListItemsResult' => ['varType' => 'any'],
                    ],
                    'httpauth' => 'Y',
                ],
            ],
