	...
	implements \Bitrix\Main\Errorable, \Bitrix\Main\Engine\Contract\Controllerable
{
	...
	public function getListAction($select = null, $order = null, $filter = null, $limit = self::DEFAULT_LIMIT)
	{
		...
		$recentTemplates = $this->getRecentTemplates($params);
		$otherTemplates = $this->getOtherTemplates($params, $recentTemplates);

		$templates = array_merge($recentTemplates, $otherTemplates);

		return array_map(
			fn(Template $item) => $item->export(empty($select) ? [] : '~'),
			$templates,
		);
	}
	...
	private function getRecentTemplates(array $params): array
	{
		$recentProvider = new TemplateRecentProvider();
		$recentIds = $recentProvider->getRecentIds($this->userId);
		...
		$params['filter']['ID'] = $recentIds;

		$recentItems = [];

		$sortedRecentItems = [];

		$recentResult = Template::find($params);
		if (!$recentResult->isSuccess())
		{
			$this->errorCollection->load($recentResult->getErrors()->transform(['DATA' => null]));
			return [];
		}

		/** @var Template $item */
		foreach ($recentResult as $item)
		{
			$recentItems[$item->getId()] = $item;
		}

		foreach ($recentIds as $id)
		{
			if (isset($recentItems[$id]))
			{
				$sortedRecentItems[] = $recentItems[$id];
			}
		}
	...
	/* @var Template[] $recentTemplates */
	private function getOtherTemplates(array $params, array $recentTemplates): array
	{
		$remainingLimit = $params['limit'] - count($recentTemplates);
		...
		$foundIds = array_map(fn($item) => $item->getId(), $recentTemplates);

		if (!empty($foundIds))
		{
			$params['filter']['!ID'] = $foundIds;
		}
