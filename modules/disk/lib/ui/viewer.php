...
final class viewer
{
     ...
     *
     * @param BaseObject|File|Folder $object           target object
     * @param array                  $additionalParams additional parameters 'relativePath', 'externalId', 'canUpdate', 'showStorage'
     *
     * @return string
     */
    public static function getAttributesByObject(BaseObject $object, array $additionalParams = [])
    {
        ...
        if ($object->getView()->getId() && $object->getView()->getJsViewerType()) {
            $dataAttributesForViewer =
                ...
                'data-bx-title="'.htmlspecialcharsbx($name).'" '.
                'data-bx-src="'.$urlManager->getUrlForShowViewHtml($object, ['sizeType' => 'absolute']).'" '.
            ...
            if ($object->getView()->getJsViewerFallbackHtmlAttributeName()) {
                $dataAttributesForViewer .= $object->getView()->getJsViewerFallbackHtmlAttributeName().'="'.$urlManager->getUrlForShowViewHtml($object, ['mode' => 'iframe']).'" ';
            }
    ...
    public static function getAttributesByExternalLink(ExternalLink $externalLink, ?BaseObject $dependedObject = null, array $additionalParams = [])
    {
        ...
        if (!$disableDocumentViewer && $object->getView()->getId() && $object->getView()->getJsViewerType()) {
            $pathToView = Driver::getInstance()->getUrlManager()->getUrlExternalLink([
                'hash' => $externalLink->getHash(),
                'action' => 'showView',
                'token' => $token,
                'path' => $path,
            ...
            $viewUrl = Driver::getInstance()->getUrlManager()->getUrlExternalLink([
                'hash' => $externalLink->getHash(),
                'action' => 'showViewHtml',
     ...
     * @return string
     */
    public static function getAttributesByVersion(Version $version, array $additionalParams = [])
    {
        ...
        if ($version->getView()->getId() && $version->getView()->getJsViewerType()) {
            $dataAttributesForViewer =
                ...
                'data-bx-src="'.$urlManager->getUrlForShowVersionViewHtml($version, ['sizeType' => 'absolute']).'" '.
            ...
            if ($version->getView()->getId() && $version->getView()->getJsViewerFallbackHtmlAttributeName()) {
                $dataAttributesForViewer .= $version->getView()->getJsViewerFallbackHtmlAttributeName().'="'.$urlManager->getUrlForShowVersionViewHtml($version, ['mode' => 'iframe']).'" ';
            }
     ...
     * @return string
     */
    public static function getAttributesByAttachedObject(AttachedObject $attachedObject, array $additionalParams = [])
    {
        $urlManager = Driver::getInstance()->getUrlManager();
        ...
        if ($object instanceof File && $object->getView()->getId() && $object->getView()->getJsViewerType()) {
            $dataAttributesForViewer =
                'data-bx-viewer="'.$object->getView()->getJsViewerType().'" '.
                'data-bx-title="'.htmlspecialcharsbx($name).'" '.
                'data-bx-src="'.$urlManager->getUrlForShowAttachedFileViewHtml($attachedObject->getId(), ['sizeType' => 'absolute'], $object->getUpdateTime()->getTimestamp()).'" '.
                ...
                'data-bx-download="'.$urlManager->getUrlUfController('download', ['attachedId' => $attachedObject->getId()]).'" '.
                'data-bx-size="'.htmlspecialcharsbx(\CFile::formatSize($size)).'" '.
                'data-bx-dateModify="'.htmlspecialcharsbx($updateTime).'" '.
                'data-bx-width="'.htmlspecialcharsbx($object->getView()->getJsViewerWidth()).'" '.
                'data-bx-height="'.htmlspecialcharsbx($object->getView()->getJsViewerHeight()).'" ';
            if ($object->getView()->getJsViewerFallbackHtmlAttributeName()) {
                $dataAttributesForViewer .= $object->getView()->getJsViewerFallbackHtmlAttributeName().'="'.$urlManager->getUrlForShowAttachedFileViewHtml($attachedObject->getId(), ['mode' => 'iframe'], $object->getUpdateTime()->getTimestamp()).'" ';
            }
            if ($object->getView()->isJsViewerHideEditButton()) {
                $dataAttributesForViewer .= 'data-bx-hideEdit="1" ';
            }
            ...
            if ($version->getView()->getJsViewerFallbackHtmlAttributeName()) {
                $dataAttributesForViewer .= $version->getView()->getJsViewerFallbackHtmlAttributeName().'="'.$urlManager->getUrlForShowAttachedVersionViewHtml($attachedObject->getId(), ['mode' => 'iframe']).'" ';
            }
            ...
            if ($object instanceof File) {
                ...
                if ($object->getView()->isTransformationAllowedOnOpen($object->getSize())) {
                    $dataAttributesForViewer .= 'data-bx-transform="'.$urlManager->getUrlUfController('transformOnOpen', ['attachedId' => $attachedObject->getId()]).'" ';
                }
            }
            ...
            $dataAttributesForViewer =
                'data-bx-viewer="image" '.
                'data-bx-title="'.htmlspecialcharsbx($name).'" '.
                'data-bx-src="'.$urlManager->getUrlUfController('download', ['attachedId' => $attachedObject->getId()]).'" '.
