...
class crmcontacts
{
    ...
    public static function processing(Block $block): void
    {
        ...
        $content = preg_replace_callback(
            ...
            static function ($matches) use ($phones, $emails) {
                $key = $matches[2];
                $sources = ('PHONE' === $matches[1]) ? $phones : $emails;
        ...
        if (false !== mb_strpos($content, '#COMPANY#')) {
    ...
    public static function getContacts(): array
    {
        if (Landing\Manager::isB24()) {
            ...
            if (0 === EntityLink::getDefaultMyCompanyId()) {
            ...
            $contacts[self::PHONES_KEY] = empty($phones) || 0 === $phones['ID']
                ? self::DEFAULT_PHONES
                : [$phones['VALUE']];
