...
class store_docs_barcode extends CCatalogStoreDocsBarcodeAll
{
    ...
    public static function getList($arOrder = [], $arFilter = [], $arGroupBy = false, $arNavStartParams = false, $arSelectFields = [])
    {
        ...
        if (empty($arSelectFields)) {
            $arSelectFields = ['ID', 'DOC_ELEMENT_ID', 'BARCODE'];
        }

        $arFields = [
            'ID' => ['FIELD' => 'DB.ID', 'TYPE' => 'int'],
            'DOC_ID' => ['FIELD' => 'DB.DOC_ID', 'TYPE' => 'int'],
            'DOC_ELEMENT_ID' => ['FIELD' => 'DB.DOC_ELEMENT_ID', 'TYPE' => 'int'],
            'BARCODE' => ['FIELD' => 'DB.BARCODE', 'TYPE' => 'string'],
        ];
