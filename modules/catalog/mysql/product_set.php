...
class product_set extends CCatalogProductSetAll
{
    public static function add($arFields)
    {
        ...
        if (0 < $intID) {
            ...
            foreach (GetModuleEvents('catalog', 'OnProductSetAdd', true) as $arEvent) {
                ExecuteModuleEventEx($arEvent, [$intID, $arFields]);
            }
    ...
    public static function update($intID, $arFields)
    {
        ...
        foreach (GetModuleEvents('catalog', 'OnBeforeProductSetUpdate', true) as $arEvent) {
            if (false === ExecuteModuleEventEx($arEvent, [$intID, &$arFields])) {
                return false;
            }
        }
    ...
    public static function delete($intID)
    {
        ...
        foreach (GetModuleEvents('catalog', 'OnBeforeProductSetDelete', true) as $arEvent) {
            if (false === ExecuteModuleEventEx($arEvent, [$intID])) {
                return false;
            }
        }
        ...
        if (!empty($arItem) && is_array($arItem)) {
            ...
            foreach (GetModuleEvents('catalog', 'OnProductSetDelete', true) as $arEvent) {
                ExecuteModuleEventEx($arEvent, [$intID]);
            }
    ...
    public static function getList($arOrder = [], $arFilter = [], $arGroupBy = false, $arNavStartParams = false, $arSelect = [])
    {
        global $DB;

        $arFields = [
            'ID' => ['FIELD' => 'CPS.ID', 'TYPE' => 'int'],
            'TYPE' => ['FIELD' => 'CPS.TYPE', 'TYPE' => 'int'],
            'SET_ID' => ['FIELD' => 'CPS.SET_ID', 'TYPE' => 'int'],
            'ACTIVE' => ['FIELD' => 'CPS.ACTIVE', 'TYPE' => 'char'],
            'OWNER_ID' => ['FIELD' => 'CPS.OWNER_ID', 'TYPE' => 'int'],
            'ITEM_ID' => ['FIELD' => 'CPS.ITEM_ID', 'TYPE' => 'int'],
            'QUANTITY' => ['FIELD' => 'CPS.QUANTITY', 'TYPE' => 'double'],
            'MEASURE' => ['FIELD' => 'CPS.MEASURE', 'TYPE' => 'int'],
            'DISCOUNT_PERCENT' => ['FIELD' => 'CPS.DISCOUNT_PERCENT', 'TYPE' => 'double'],
            'SORT' => ['FIELD' => 'CPS.SORT', 'TYPE' => 'int'],
            'CREATED_BY' => ['FIELD' => 'CPS.CREATED_BY', 'TYPE' => 'int'],
            'DATE_CREATE' => ['FIELD' => 'CPS.DATE_CREATE', 'TYPE' => 'datetime'],
            'MODIFIED_BY' => ['FIELD' => 'CPS.MODIFIED_BY', 'TYPE' => 'int'],
            'TIMESTAMP_X' => ['FIELD' => 'CPS.TIMESTAMP_X', 'TYPE' => 'datetime'],
            'XML_ID' => ['FIELD' => 'CPS.XML_ID', 'TYPE' => 'string'],
        ];
    ...
    public static function getAllSetsByProduct($intProductID, $intSetType)
    {
        ...
        while ($arItem = $rsItems->Fetch()) {
            ...
            if ($boolSet) {
                $arItem['QUANTITY'] = (null === $arItem['QUANTITY'] ? false : (float) $arItem['QUANTITY']);
                $arItem['MEASURE'] = (null === $arItem['MEASURE'] ? false : (int) $arItem['MEASURE']);
                $arItem['DISCOUNT_PERCENT'] = (null === $arItem['DISCOUNT_PERCENT'] ? false : $arItem['DISCOUNT_PERCENT']);
            }
            if ($boolProduct) {
                unset($arItem['OWNER_ID'], $arItem['SET_ID'], $arItem['ID']);
            } else {
                unset($arItem['SET_ID'], $arItem['OWNER_ID'], $arItem['ACTIVE']);
            }
    ...
    public static function getSetByID($intID)
    {
        ...
        if ($arItem = $rsItems->Fetch()) {
            ...
            $arItem['QUANTITY'] = (null === $arItem['QUANTITY'] ? false : (float) $arItem['QUANTITY']);
            $arItem['MEASURE'] = (null === $arItem['MEASURE'] ? false : (int) $arItem['MEASURE']);
            $arItem['DISCOUNT_PERCENT'] = (null === $arItem['DISCOUNT_PERCENT'] ? false : (float) $arItem['DISCOUNT_PERCENT']);
            ...
            while ($arSub = $rsSubs->Fetch()) {
                ...
                $arSub['QUANTITY'] = (null === $arSub['QUANTITY'] ? false : (float) $arSub['QUANTITY']);
                $arSub['MEASURE'] = (null === $arSub['MEASURE'] ? false : (int) $arSub['MEASURE']);
                $arSub['DISCOUNT_PERCENT'] = (null === $arSub['DISCOUNT_PERCENT'] ? false : (float) $arSub['DISCOUNT_PERCENT']);
    ...
    protected static function calculateSetParams($productID, $items)
    {
        ...
        foreach ($tracedItems as &$oneItem) {
            ...
            if (null === $quantity || $quantity > $itemQuantity) {
        ...
        $fields['AVAILABLE'] = (
            CCatalogProduct::isAvailable($fields)
        ...
        foreach (GetModuleEvents('catalog', 'OnBeforeProductSetAvailableUpdate', true) as $arEvent) {
            ExecuteModuleEventEx($arEvent, [$productID, &$fields]);
        }
    ...
    protected static function createSetItemsParamsFromUpdate($setID, $getProductID = false)
    {
        ...
        $getProductID = (true === $getProductID);
        if ($getProductID) {
            $result = [
                'ITEM_ID' => 0,
                'ITEMS' => [],
