    ...
    protected static function checkFields($action, &$arFields, $id = 0)
    {
        ...
        switch ($action) {
            case 'ADD':
                ...
                $cnt = CCatalogMeasure::getList([], ['CODE' => $arFields['CODE']], []);

                break;

            case 'UPDATE':
                if (isset($arFields['CODE'])) {
                    $cnt = CCatalogMeasure::getList([], ['CODE' => $arFields['CODE'], '!ID' => $id], [], false, ['ID']);
                }
        ...
        if (isset($arFields['IS_DEFAULT']) && 'Y' === $arFields['IS_DEFAULT']) {
            ...
            while ($row = $iterator->fetch()) {
                $result = Catalog\MeasureTable::update((int) $row['ID'], ['IS_DEFAULT' => 'N']);
                if (!$result->isSuccess()) {
                    return false;
                }
            }
