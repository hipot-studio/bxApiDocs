...
class discount
{
    ...
    protected static $arCacheProduct = [];
    protected static $arCacheDiscountFilter = [];
    protected static $arCacheDiscountResult = [];
    protected static $arCacheProductSectionChain = [];
    protected static $arCacheProductSections = [];
    protected static $arCacheProductProperties = [];
    protected static $cacheDiscountHandlers = [];
    protected static $usedModules = [];
                    ...
                    if (isset($arParams['PRODUCT']) && 'Y' === $arParams['PRODUCT']) {
                        ...
                        if (!isset($arProduct['SECTION_ID'])) {
                            ...
                            if (false !== $arProductSections) {
                                ...
                                $arProduct['SECTION_ID'] = [];
                            }
                        } else {
                            if (!is_array($arProduct['SECTION_ID'])) {
                                $arProduct['SECTION_ID'] = [$arProduct['SECTION_ID']];
                            }
                        ...
                        if (!empty($arProduct['DATE_ACTIVE_FROM'])) {
                            ...
                            if ($arProduct['DATE_ACTIVE_FROM'].'!' !== $intStackTimestamp.'!') {
                        ...
                        if (!empty($arProduct['DATE_ACTIVE_TO'])) {
                            ...
                            if ($arProduct['DATE_ACTIVE_TO'].'!' !== $intStackTimestamp.'!') {
                        ...
                        if (!empty($arProduct['DATE_CREATE'])) {
                            ...
                            if ($arProduct['DATE_CREATE'].'!' !== $intStackTimestamp.'!') {
                        ...
                        if (!empty($arProduct['TIMESTAMP_X'])) {
                            ...
                            if ($arProduct['TIMESTAMP_X'].'!' !== $intStackTimestamp.'!') {
                        ...
                        if ('' !== $arProduct['CATALOG_QUANTITY']) {
                            $arProduct['CATALOG_QUANTITY'] = (float) $arProduct['CATALOG_QUANTITY'];
                        }
                        ...
                        if ('' !== $arProduct['CATALOG_WEIGHT']) {
                            $arProduct['CATALOG_WEIGHT'] = (float) $arProduct['CATALOG_WEIGHT'];
                        }
                        ...
                        $arSelect = ['ID', 'IBLOCK_ID', 'CODE', 'XML_ID', 'NAME', 'ACTIVE', 'DATE_ACTIVE_FROM', 'DATE_ACTIVE_TO',
                        ...
                        CTimeZone::Disable();
                        $rsProducts = CIBlockElement::GetList([], ['ID' => $arProduct['ID'], 'IBLOCK_ID' => $arProduct['IBLOCK_ID']], false, false, $arSelect);
                        CTimeZone::Enable();
                        ...
                        if (!isset(self::$arCacheProductProperties[$arProduct['ID']])) {
                            $arProps = $obProduct->GetProperties([], ['ACTIVE' => 'Y', 'EMPTY' => 'N']);
                        } else {
                            $arProps = self::$arCacheProductProperties[$arProduct['ID']];
                        }
                        self::__ConvertProperties($arProduct, $arProps, ['TIME_ZONE' => 'N']);
                        if (isset(self::$arCacheProductProperties[$arProduct['ID']])) {
    ...
    protected static function __GetSectionList($intIBlockID, $intProductID)
    {
        ...
        if ($intIBlockID > 0 && $intProductID > 0) {
            ...
            if (!isset(self::$arCacheProductSections[$intProductID])) {
                $rsSections = CIBlockElement::GetElementGroups($intProductID, true, ['ID', 'IBLOCK_SECTION_ID', 'IBLOCK_ELEMENT_ID']);
                ...
                foreach ($arProductSections as &$arSection) {
                    ...
                    if (0 < $arSection['IBLOCK_SECTION_ID']) {
                        if (!isset(self::$arCacheProductSectionChain[$arSection['ID']])) {
                            self::$arCacheProductSectionChain[$arSection['ID']] = [];
                            $rsParents = CIBlockSection::GetNavChain($intIBlockID, $arSection['ID'], ['ID']);
    ...
    protected static function __ConvertProperties(&$arProduct, &$arProps, $arParams = false)
    {
        ...
        if (!isset($iblockProperties[$arProduct['IBLOCK_ID']])) {
            $iblockProperties[$arProduct['IBLOCK_ID']] = [];
            $propertyIterator = Iblock\PropertyTable::getList([
                'select' => ['ID', 'IBLOCK_ID', 'CODE', 'PROPERTY_TYPE', 'MULTIPLE', 'USER_TYPE', 'USER_TYPE_SETTINGS'],
                'filter' => ['=IBLOCK_ID' => $arProduct['IBLOCK_ID'], '=ACTIVE' => 'Y', '!=PROPERTY_TYPE' => Iblock\PropertyTable::TYPE_FILE],
                'order' => ['ID' => 'ASC'],
            ]);
        ...
        foreach ($arProps as &$arOneProp) {
            if (Iblock\PropertyTable::TYPE_FILE === $arOneProp['PROPERTY_TYPE']) {
                continue;
            }
            $propertyId = (int) $arOneProp['ID'];
            if (isset($propertyList[$propertyId])) {
                unset($propertyList[$propertyId], $propertyId);
            }
                    ...
                    switch ($arOneProp['USER_TYPE']) {
                        ...
                        case 'Date':
                            ...
                            if ('' !== $arOneProp['VALUE']) {
                                ...
                                if ('DateTime' === $arOneProp['USER_TYPE']) {
                                ...
                                if ($arOneProp['VALUE'].'!' !== $intStackTimestamp.'!') {
            ...
            if (!is_array($arProduct['PROPERTY_'.$arOneProp['ID'].'_VALUE'])) {
                $arProduct['PROPERTY_'.$arOneProp['ID'].'_VALUE'] = [$arProduct['PROPERTY_'.$arOneProp['ID'].'_VALUE']];
            }
        }
        unset($arOneProp);
    ...
    protected static function __GenerateParent(&$product, $sku)
    {
        ...
        if (!isset(self::$arCacheProduct[$parentID])) {
            $parent = ['ID' => $parentID, 'IBLOCK_ID' => $sku['PRODUCT_IBLOCK_ID']];
        ...
        foreach ($parent as $key => $value) {
            ...
            } elseif (0 === strncmp($key, 'PROPERTY_', 9)) {
                $product[$key] = $value;
            } elseif (0 !== strncmp($key, 'CATALOG_', 8)) {
    ...
    protected static function __ParseArrays(&$arFields)
    {
        ...
        if (!self::__CheckOneEntity($arFields, 'GROUP_IDS')) {
            $arMsg[] = ['id' => 'GROUP_IDS', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_PARSE_USER_GROUP')];
            $boolResult = false;
        }
        if (!self::__CheckOneEntity($arFields, 'CATALOG_GROUP_IDS')) {
            $arMsg[] = ['id' => 'CATALOG_GROUP_IDS', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_PARSE_PRICE_TYPE')];
            $boolResult = false;
        }
            ...
            if (is_array($arFields) && !empty($arFields)) {
                if (is_set($arFields, $strEntityID)) {
                    if (!is_array($arFields[$strEntityID])) {
                        $arFields[$strEntityID] = [$arFields[$strEntityID]];
                    }
                    $arValid = [];
    ...
    protected static function __ArrayMultiple($arOrder, &$arResult, $arTuple, $arTemp = [])
    {
        if (empty($arTuple)) {
            $arResult[] = [
    ...
    protected static function __GetConditionValues(&$arFields)
    {
        ...
        if (isset($arFields['CONDITIONS']) && !empty($arFields['CONDITIONS'])) {
            ...
            if (is_array($arConditions) && !empty($arConditions)) {
                ...
                $boolCond = $obCond->Init(BT_COND_MODE_SEARCH, BT_COND_BUILD_CATALOG, []);
                if ($boolCond) {
    ...
    public static function execAgent(): string
    {
        if (
            ...
            || 'N' !== (string) Option::get('sale', 'use_sale_discount_only')) {
            return '';
        }
    ...
    public static function setSaleDiscountFilter($priceTypesOnly = false)
    {
        ...
        if (self::$useSaleDiscount) {
            self::$getPriceTypesOnly = (true === $priceTypesOnly);
        }
    }
    ...
    {
        if (null === self::$getPercentFromBasePrice) {
            self::initDiscountSettings();
        }
    ...
    {
        if (true !== $useBasePrice && false !== $useBasePrice) {
            return;
        }
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if ('ADD' === $ACTION) {
            ...
            if (!isset($arFields['SITE_ID'])) {
                $boolResult = false;
                $arMsg[] = ['id' => 'SITE_ID', 'text' => Loc::getMessage('KGD_EMPTY_SITE')];
            }
            if (!isset($arFields['CURRENCY'])) {
                $boolResult = false;
                $arMsg[] = ['id' => 'CURRENCY', 'text' => Loc::getMessage('KGD_EMPTY_CURRENCY')];
            }
            if (!isset($arFields['NAME'])) {
                $boolResult = false;
                $arMsg[] = ['id' => 'NAME', 'text' => Loc::getMessage('KGD_EMPTY_NAME')];
            }
            if (!isset($arFields['VALUE'])) {
                $boolResult = false;
                $arMsg[] = ['id' => 'VALUE', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_BAD_VALUE')];
            }
            if (!isset($arFields['CONDITIONS'])) {
                $boolResult = false;
                $arMsg[] = ['id' => 'CONDITIONS', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_EMPTY_CONDITIONS')];
            }
        ...
        if ('UPDATE' === $ACTION) {
            ...
            if ($ID <= 0) {
                $boolResult = false;
                $arMsg[] = ['id' => 'ID', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_BAD_ID', ['#ID#', $ID])];
            } else {
                ...
                if ($boolValueType !== $boolValue) {
                    ...
                    if ($arCurrent = $rsDiscounts->Fetch()) {
                        $arCurrent['VALUE'] = (float) $arCurrent['VALUE'];
                    } else {
                        $boolResult = false;
                        $arMsg[] = ['id' => 'ID', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_BAD_ID', ['#ID#', $ID])];
                    }
        ...
        if ($boolResult) {
            ...
            if (isset($arFields['MAX_DISCOUNT'])) {
                ...
                $arFields['MAX_DISCOUNT'] = (float) $arFields['MAX_DISCOUNT'];
            ...
            if ($boolValueType) {
                if (!in_array($arFields['VALUE_TYPE'], CCatalogDiscount::GetDiscountTypes(), true)) {
                    $arFields['VALUE_TYPE'] = self::TYPE_PERCENT;
                }
            }
            if ($boolValue) {
                $arFields['VALUE'] = str_replace(',', '.', $arFields['VALUE']);
                $arFields['VALUE'] = (float) $arFields['VALUE'];
                if ($arFields['VALUE'] <= 0) {
                    $boolResult = false;
                    $arMsg[] = ['id' => 'VALUE', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_BAD_VALUE')];
                }
            ...
            if ($boolValue && $boolValueType) {
                if (self::TYPE_PERCENT === $arFields['VALUE_TYPE'] && $arFields['VALUE'] > 100) {
                    $boolResult = false;
                    $arMsg[] = ['id' => 'VALUE', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_BAD_VALUE')];
                }
            }
            if (isset($arFields['MIN_ORDER_SUM'])) {
                ...
                $arFields['MIN_ORDER_SUM'] = (float) $arFields['MIN_ORDER_SUM'];
            }
            ...
            if (isset($arFields['USE_COUPONS'])) {
                $arFields['USE_COUPONS'] = ('Y' !== $arFields['USE_COUPONS'] ? 'N' : 'Y');
            }
        }
        if ($boolResult) {
            if (isset($arFields['CONDITIONS'])) {
                if (empty($arFields['CONDITIONS'])) {
                    ...
                    if (!is_array($arFields['CONDITIONS'])) {
                        if (!CheckSerializedData($arFields['CONDITIONS'])) {
                            $boolCond = false;
                            $boolResult = false;
                            $arMsg[] = ['id' => 'CONDITIONS', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_BAD_CONDITIONS')];
                        } else {
                    ...
                    if ($boolCond) {
                        $obCond = new CCatalogCondTree();
                        $boolCond = $obCond->Init(BT_COND_MODE_GENERATE, BT_COND_BUILD_CATALOG, []);
                        if (!$boolCond) {
                            return false;
                        }
                        $strEval = $obCond->Generate($arFields['CONDITIONS'], ['FIELD' => '$arProduct']);
                    ...
                    if ($boolCond) {
                        ...
                        if ('MYSQL' === $DB->type) {
                            if (64_000 < strlen($arFields['UNPACK']) || 64_000 < strlen($arFields['CONDITIONS'])) {
                                $boolResult = false;
                                $arMsg[] = ['id' => 'CONDITIONS', 'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_CONDITIONS_TOO_LONG')];
    ...
    public static function Add($arFields)
    {
        ...
        if (isset($arFields['CATALOG_COUPONS'])) {
            ...
            if (!is_array($arFields['CATALOG_COUPONS'][$arKeys[0]])) {
                $arFields['CATALOG_COUPONS'] = [$arFields['CATALOG_COUPONS']];
            }
    ...
    public static function OnCurrencyDelete($Currency)
    {
        ...
        $dbDiscounts = CCatalogDiscount::GetList([], ['CURRENCY' => $Currency], false, false, ['ID']);
    ...
    public static function GetDiscountByPrice($productPriceID, $arUserGroups = [], $renewal = 'N', $siteID = false, $arDiscountCoupons = false)
    {
        global $APPLICATION;

        foreach (GetModuleEvents('catalog', 'OnGetDiscountByPrice', true) as $arEvent) {
            $mxResult = ExecuteModuleEventEx($arEvent, [$productPriceID, $arUserGroups, $renewal, $siteID, $arDiscountCoupons]);
            if (true !== $mxResult) {
                return $mxResult;
            }
        }
        ...
        $dbPrice = CPrice::GetListEx(
            ...
            false,
            false,
            ['ID', 'PRODUCT_ID', 'CATALOG_GROUP_ID', 'ELEMENT_IBLOCK_ID']
        );
    ...
    public static function GetDiscountByProduct($productID = 0, $arUserGroups = [], $renewal = 'N', $arCatalogGroups = [], $siteID = false, $arDiscountCoupons = false)
    {
        global $APPLICATION;

        foreach (GetModuleEvents('catalog', 'OnGetDiscountByProduct', true) as $arEvent) {
            $mxResult = ExecuteModuleEventEx($arEvent, [$productID, $arUserGroups, $renewal, $arCatalogGroups, $siteID, $arDiscountCoupons]);
            if (true !== $mxResult) {
                return $mxResult;
            }
        }
     ...
     * @return array|false
     */
    public static function GetDiscount($intProductID, $intIBlockID, $arCatalogGroups = [], $arUserGroups = [], $strRenewal = 'N', $siteID = false, $arDiscountCoupons = false, $boolSKU = true, $boolGetIDS = false)
    {
        ...
        if (true === $eventOnGetExists || null === $eventOnGetExists) {
            foreach (GetModuleEvents('catalog', 'OnGetDiscount', true) as $arEvent) {
                ...
                $mxResult = ExecuteModuleEventEx($arEvent, [$intProductID, $intIBlockID, $arCatalogGroups, $arUserGroups, $strRenewal, $siteID, $arDiscountCoupons, $boolSKU, $boolGetIDS]);
                if (true !== $mxResult) {
                    return $mxResult;
                }
            }
        ...
        $boolSKU = (true === $boolSKU);
        $boolGetIDS = (true === $boolGetIDS);
        ...
        if (!is_array($arCatalogGroups)) {
            $arCatalogGroups = [$arCatalogGroups];
        }
        ...
        if (!empty($arCatalogGroups)) {
            $emptyExist = in_array(-1, $arCatalogGroups, true);
            Main\Type\Collection::normalizeArrayValuesByInt($arCatalogGroups, true);
            if ($emptyExist) {
                $arCatalogGroups = array_merge([-1], $arCatalogGroups);
            }
            unset($emptyExist);
        }
        if (empty($arCatalogGroups)) {
        ...
        $strRenewal = ('Y' === (string) $strRenewal ? 'Y' : 'N');
        ...
        if (self::$useSaleDiscount && Loader::includeModule('sale')) {
            ...
            /** @var DiscountCouponsManager $couponManagerClass */
            ...
            if (self::$needDiscountCache[$cacheIndex] && $arCatalogGroups !== [-1]) {
                ...
                if (false === $arDiscountCoupons) {
                    if (self::$existCouponsManager && Loader::includeModule('sale')) {
                        $arDiscountCoupons = DiscountCouponsManager::getForApply(
                            ['MODULE_ID' => 'catalog', 'DISCOUNT_ID' => $arDiscountIDs],
                            ['MODULE_ID' => 'catalog', 'PRODUCT_ID' => $intProductID, 'BASKET_ID' => '0'],
                            true
                        );
                    ...
                    if (self::$existCouponsManager && Loader::includeModule('sale')) {
                        $orderCoupons = DiscountCouponsManager::getOrderedCoupons(true, ['COUPON' => $arDiscountCoupons]);
                    }
                }
                if (false === $arDiscountCoupons) {
                    $arDiscountCoupons = [];
                ...
                if (!isset(self::$arCacheDiscountResult[$strCacheKey])) {
                    ...
                    if (!empty($arDiscountCoupons) && is_array($arDiscountCoupons)) {
                        $iterator = Catalog\DiscountCouponTable::getList([
                            'select' => ['DISCOUNT_ID', 'COUPON', 'ACTIVE', 'TYPE'],
                            'filter' => ['@DISCOUNT_ID' => $arDiscountIDs, '@COUPON' => $arDiscountCoupons],
                ...
                if (!empty($arDiscountList)) {
                    ...
                    foreach ($arDiscountList as &$arPriceDiscount) {
                        ...
                        if (
                            ...
                            && 'N' === $arPriceDiscount['COUPON_ACTIVE']
                        ...
                        if (!$boolGenerate) {
                            if (!isset(self::$arCacheProduct[$intProductID])) {
                                $arProduct = ['ID' => $intProductID, 'IBLOCK_ID' => $intIBlockID];
                    ...
                    unset($arPriceDiscount, $discountApply);
                }
            }
        ...
        if (true === $eventOnResultExists || null === $eventOnResultExists) {
            foreach (GetModuleEvents('catalog', 'OnGetDiscountResult', true) as $arEvent) {
                ...
                ExecuteModuleEventEx($arEvent, [&$arResult]);
            }
    ...
    public static function HaveCoupons($ID, $excludeID = 0)
    {
        ...
        $dbRes = CCatalogDiscountCoupon::GetList([], $arFilter, false, ['nTopCount' => 1], ['ID']);
        if ($dbRes->Fetch()) {
            return true;
        }
    ...
    public static function GetDiscountForProduct($arProduct, $arParams = false)
    {
        ...
        if (self::$useSaleDiscount && Loader::includeModule('sale')) {
            ...
            $arSelect = ['ID', 'SITE_ID', 'SORT', 'NAME', 'VALUE_TYPE', 'VALUE', 'MAX_DISCOUNT', 'CURRENCY', 'UNPACK', 'NOTES'];
            ...
            if (!in_array('UNPACK', $arSelect, true)) {
                $arSelect[] = 'UNPACK';
            }
    ...
    {
        $boolKeys = (bool) $boolKeys;
        $boolRevert = (bool) $boolRevert;
        if (!is_array($arParams) || empty($arParams)) {
            return [];
        }
        $arFilter = ['RESTRICTIONS' => true];
    ...
    public static function applyDiscountList($price, $currency, &$discountList)
    {
        ...
        if (false === $currency || !is_array($discountList)) {
            return false;
        }

        if (null === self::$useSaleDiscount) {
            self::initDiscountSettings();
        }
        ...
        if (!empty($productDiscountList)) {
            if (self::$useSaleDiscount && Loader::includeModule('sale')) {
                Price\Calculation::pushConfig();
                Price\Calculation::setConfig([
                    'PRECISION' => (int) Option::get('sale', 'value_precision'),
                ]);
                $applyDiscountList = [];
                foreach ($productDiscountList as $priority => $discounts) {
                    foreach ($discounts as $discount) {
                        ...
                        if (false !== $currentPrice) {
                            ...
                            if ('Y' === $discount['LAST_DISCOUNT']) {
                                break 2;
                            }
                            if ('Y' === $discount['LAST_LEVEL_DISCOUNT']) {
                                break;
                            }
                        }
                    }
                ...
                foreach ($productDiscountList as &$priority) {
                    ...
                    if (!empty($applyDiscountList)) {
                        ...
                        if (isset($lastDiscount['LAST_DISCOUNT']) && 'Y' === $lastDiscount['LAST_DISCOUNT']) {
                            break;
                        }
                    }
                }
    ...
    public static function calculateDiscountList($priceData, $currency, &$discountList, $getWithVat = true)
    {
        ...
        $currency = CCurrency::checkCurrencyID($currency);
        if (false === $priceData['CURRENCY'] || false === $currency || !is_array($discountList)) {
            return $result;
        }
        ...
        unset($vatRate, $priceData['ORIG_VAT_INCLUDED']);

        $unroundBasePrice = $calculatePrice;
        $unroundPrice = $currentPrice;
        if (Price\Calculation::isComponentResultMode()) {
            $calculatePrice = Price::roundPrice(
                $priceData['CATALOG_GROUP_ID'],
                $calculatePrice,
                $currency
            );
            $currentPrice = Price::roundPrice(
                $priceData['CATALOG_GROUP_ID'],
                $currentPrice,
                $currency
            );
            if (
                empty($discountList)
                || Price\Calculation::compare($result['BASE_PRICE'], $result['PRICE'], '<=')
            ) {
    ...
    public static function getDiscountDescription(array $discount)
    {
        $result = [
            ...
            'MAX_VALUE' => (
                Catalog\DiscountTable::VALUE_TYPE_PERCENT === $discount['VALUE_TYPE']
                ? $discount['MAX_DISCOUNT']
                : 0
            ),
            'CURRENCY' => $discount['CURRENCY'],
            'HANDLERS' => (isset($discount['HANDLERS']) ? $discount['HANDLERS'] : []),
        ];
            ...
            if (!empty($arIDS)) {
                ...
                while ($arItem = $rsItems->Fetch()) {
                    ...
                    if ($boolFields) {
                        ...
                        if (false !== $arProductSections) {
                            ...
                            $arBasketData['SECTION_ID'] = [];
                        }
                    }
                    if ($boolProps) {
                        $arBasketData['PROPERTIES'] = [];
                    }
                    $arBasketResult[$arItem['ID']] = $arBasketData;
                }
                $propertyFilter = [];
                if (isset($arExtend['iblock']['props']) && is_array($arExtend['iblock']['props'])) {
                    $propertyFilter = ['ID' => $arExtend['iblock']['props']];
                }
                if (!empty($iblockGroup)) {
                    $catalogIterator = Catalog\CatalogIblockTable::getList([
                        'select' => ['IBLOCK_ID', 'SKU_PROPERTY_ID', 'PRODUCT_IBLOCK_ID'],
                        'filter' => ['@IBLOCK_ID' => array_keys($iblockGroup)],
                    ]);
                            ...
                            if (!empty($iblockGroup[$catalog['IBLOCK_ID']])) {
                                foreach ($iblockGroup[$catalog['IBLOCK_ID']] as $itemId) {
                                    if (!isset($arBasketResult[$itemId]['PROPERTIES'])) {
                                        $arBasketResult[$itemId]['PROPERTIES'] = [];
                                    }
                                }
                                unset($itemId);
                            }
                ...
                if ($boolProps && !empty($iblockGroup)) {
                    ...
                    foreach ($arBasketResult as &$basketItem) {
                        self::__ConvertProperties($basketItem, $basketItem['PROPERTIES'], ['TIME_ZONE' => 'N']);
                    }
                    unset($basketItem);
                }
                ...
                if (!empty($offerIds)) {
                    ...
                    if (!empty($products)) {
                        ...
                        foreach (array_keys($products) as $iblockId) {
                            ...
                            CCatalogDiscount::SetDiscountProductCache($productIds[$iblockId], ['IBLOCK_ID' => $iblockId, 'GET_BY_ID' => 'Y']);
                        }
    ...
    public static function SetAllDiscountFilterCache($arDiscountCache, $boolNeedClear = true)
    {
        ...
        $boolNeedClear = (bool) $boolNeedClear;
    ...
    public static function SetDiscountProductCache($arItem, $arParams = [])
    {
        ...
        if (!empty($arParams) && isset($arParams['GET_BY_ID']) && 'Y' === $arParams['GET_BY_ID']) {
            $filter = ['ID' => $arItem];
            if (isset($arParams['IBLOCK_ID'])) {
                $filter['IBLOCK_ID'] = $arParams['IBLOCK_ID'];
            }

            $select = ['ID', 'IBLOCK_ID', 'CODE', 'XML_ID', 'NAME', 'ACTIVE', 'DATE_ACTIVE_FROM', 'DATE_ACTIVE_TO',
            ...
            while ($arProductFields = $rsProducts->GetNext(false, true)) {
                ...
                if (!isset(self::$arCacheProductProperties[$arProduct['ID']])) {
                    ...
                    CIBlockElement::GetPropertyValuesArray(
                        $propsList,
                        $arProduct['IBLOCK_ID'],
                        ['ID' => $arProduct['ID'], 'IBLOCK_ID' => $arProduct['IBLOCK_ID']],
                        [],
                        [
                            'ID' => true,
                            'PROPERTY_TYPE' => true,
    ...
    public static function SetProductSectionsCache($arItemIDs)
    {
        ...
        if (empty(self::$arCacheProductSections)) {
            self::$arCacheProductSections = array_fill_keys($arItemIDs, []);
        } else {
            foreach ($arItemIDs as $intOneID) {
                self::$arCacheProductSections[$intOneID] = [];
            }
            unset($intOneID);
        }
        ...
        foreach (array_chunk($arItemIDs, 500) as $pageIds) {
            $rsSections = CIBlockElement::GetElementGroups(
                $pageIds,
                true,
                ['ID', 'IBLOCK_SECTION_ID', 'IBLOCK_ELEMENT_ID']
            );
            while ($arSection = $rsSections->Fetch()) {
                $arSection['ID'] = (int) $arSection['ID'];
                $arSection['IBLOCK_SECTION_ID'] = (int) $arSection['IBLOCK_SECTION_ID'];
    ...
    public static function ClearDiscountCache($arTypes)
    {
        ...
        if (isset($arTypes['SECTIONS'])) {
            self::$arCacheProductSections = [];
        }
        if (isset($arTypes['SECTION_CHAINS'])) {
            self::$arCacheProductSectionChain = [];
        }
        if (isset($arTypes['PROPERTIES'])) {
            self::$arCacheProductProperties = [];
        }
    }
    ...
    protected static function getAllUserGroups()
    {
        ...
        if (!$groups) {
            $dbGroupsList = CGroup::GetListEx(['ID' => 'DESC'], ['ACTIVE' => 'Y']);
    ...
    protected static function primaryDiscountFilter($price, $currency, &$discountList, &$priceDiscountList, &$accumulativeDiscountList)
    {
        ...
        foreach ($discountList as $oneDiscount) {
            ...
            switch ($oneDiscount['VALUE_TYPE']) {
                ...
                case self::TYPE_PERCENT:
                    ...
                    if ($validDiscount) {
                        ...
                        if (self::ENTITY_ID === $oneDiscount['TYPE'] && $oneDiscount['MAX_DISCOUNT'] > 0) {
            ...
            if (CCatalogDiscountSave::ENTITY_ID === $oneDiscount['TYPE']) {
                $accumulativeDiscountList[] = $oneDiscount;
            } elseif (self::ENTITY_ID === $oneDiscount['TYPE']) {
                if (!isset($priceDiscountList[$oneDiscount['PRIORITY']])) {
                    $priceDiscountList[$oneDiscount['PRIORITY']] = [];
                }
    ...
    protected static function calculatePriorityLevel($basePrice, $price, $currency, &$discountList, &$resultDiscount)
    {
        ...
        $price = (float) $price;
        $currency = CCurrency::checkCurrencyID($currency);
        if ($basePrice <= 0 || $price <= 0 || false === $currency) {
            return false;
        }
        ...
        do {
            ...
            foreach ($discountList as $discountIndex => $oneDiscount) {
                ...
                if ($needErase) {
                    ...
                    $apply = (false === $minPrice || $minPrice > $calculatePrice);
                    if ($apply) {
            ...
            if (false !== $minPrice) {
                ...
                if ('Y' === $discountList[$minIndex]['LAST_DISCOUNT']) {
                    $discountList = [];
                } else {
    ...
    protected static function calculateDiscSave($basePrice, $price, $currency, &$discsaveList, &$resultDiscount)
    {
        ...
        if (false !== $minPrice && isset($discsaveList[$minIndex])) {
            $currentPrice = $minPrice;
    ...
    protected static function initDiscountSettings()
    {
        ...
        if (null === self::$useSaleDiscount) {
            self::$useSaleDiscount = $saleInstalled && 'Y' === (string) Option::get('sale', 'use_sale_discount_only');
        }
        if (null === self::$getPercentFromBasePrice) {
            $moduleID = ($saleInstalled ? 'sale' : 'catalog');
            self::$getPercentFromBasePrice = 'Y' === (string) Option::get($moduleID, 'get_discount_percent_from_base_price');
        }
    ...
    private static function getDiscountsFromApplyResult(array $calcResults, BasketItemBase $basketItem)
    {
        $finalDiscountList = [];

        if ($calcResults && !empty($calcResults['PRICES']['BASKET']) && 1 === count($calcResults['PRICES']['BASKET'])) {
    ...
    private static function getReformattedDiscounts(array $finalDiscountList, array $calcResults, $siteId, $isRenewal = false)
    {
        ...
        foreach ($finalDiscountList as $discount) {
            if ($discount['SHORT_DESCRIPTION_STRUCTURE']) {
                ...
                $actionConfiguration = Actions::getActionConfiguration($discount);
            }

            if (!$actionConfiguration || Actions::VALUE_TYPE_SUMM === $actionConfiguration['VALUE_TYPE']) {
                continue;
            }

            if ('Extra' === $actionConfiguration['TYPE']) {
                continue;
            }

            if ('Closeout' === $actionConfiguration['TYPE']) {
            ...
            if ('Y' === $discount['USE_COUPONS']) {
                foreach ($calcResults['COUPON_LIST'] as $coupon) {
                    ...
                    if (DiscountCouponTable::TYPE_BASKET_ROW === $coupon['TYPE']) {
                        $reformattedDiscount['COUPON_ONE_TIME'] = 'Y';
                    } elseif (DiscountCouponTable::TYPE_ONE_ORDER === $coupon['TYPE']) {
                        $reformattedDiscount['COUPON_ONE_TIME'] = 'O';
                    } elseif (DiscountCouponTable::TYPE_MULTI_ORDER === $coupon['TYPE']) {
    ...
    private static function getSaleDiscountsByProduct(array $product, $siteId, array $userGroups, array $priceRow, $isRenewal, $coupons)
    {
        ...
        if (null !== $basket) {
            $orderedBasket = null !== $basket->getOrder();
        ...
        $config = Price\Calculation::getConfig();
        if (null !== $config['CURRENCY'] && $config['CURRENCY'] !== $priceRow['CURRENCY']) {
