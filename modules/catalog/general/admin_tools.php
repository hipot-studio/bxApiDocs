...
class CCatalogAdminTools extends CCatalogAdminToolsAll
{
    ...
    public static function getCurrentTabFromRequest(): ?string
    {
        ...
        if (!Catalog\Config\Feature::isProductSetsEnabled()) {
            unset($tabList[self::TAB_SET], $tabList[self::TAB_GROUP]);
        }
    ...
    public static function getIBlockElementMenu(
        ...
        $publicShop = !(
            Iblock\Url\AdminPage\IblockBuilder::TYPE_ID === $builderId
            || Catalog\Url\AdminPage\CatalogBuilder::TYPE_ID === $builderId
        );
        if ($publicShop) {
            // TODO: need fix this hack
            if ('CRM' === $builderId) {
                if (Loader::includeModule('crm')) {
                    $productCardEnabled = LayoutSettings::getCurrent()->isFullCatalogEnabled();
                }
            } else {
        ...
        if (!empty($productLimits)) {
            ...
            $publicFlag = $productCardEnabled
                || Catalog\Url\InventoryBuilder::TYPE_ID === $builderId; // hack for inventory documents

            $serviceItem = null;
            if (
                'N' === $arCatalog['SUBSCRIPTION']
                && (
            ...
            if ('Y' === $arCatalog['CATALOG']) {
                if ($productCardEnabled) {
                    ...
                    } elseif (CCatalogSku::TYPE_FULL === $arCatalog['CATALOG_TYPE']) {
                        $additionalParams = [
                ...
                if (!isset($arParams['from']) || 'iblock_section_admin' !== $arParams['from']) {
                    $arItems[] = [
                        'ICON' => 'btn_new',
                        'TEXT' => Loc::getMessage('BT_CAT_ADM_TOOLS_ADD_PROD_EXT_2'),
            ...
            if (
                ...
                if (Instagram::isAvailable()
                    ...
                    $arItems[] = [
                        ...
                        'LINK' => Option::get('crm', 'path_to_order_import_instagram'),
                        'PUBLIC' => true,
                        'SHOW_TITLE' => true,
                    ];
                }
    ...
    public static function getIBlockElementContentMenu(
        ...
        if (null !== $product) {
            ...
            $boolExistSet = Catalog\ProductTable::TYPE_SET === $productType;
            $boolExistGroup = Catalog\ProductTable::STATUS_YES === $product['BUNDLE'];
        }
        if (!$boolExistSet) {
        ...
        if (!$existInSet) {
            if (
                ...
                if (
                    Catalog\ProductTable::TYPE_SET === $productType
                    && null === $currentTab
                ) {
                    ...
                    Catalog\ProductTable::TYPE_SKU === $productType
                    && null === $currentTab
                ) {
                    ...
                    (
                        Catalog\ProductTable::TYPE_PRODUCT === $productType
                        && null === $currentTab
                    )
                    || self::TAB_CATALOG === $currentTab
                ) {
                    $row['CHECKED'] = true;
                } else {
            ...
            if (
                ...
                if (Catalog\ProductTable::TYPE_EMPTY_SKU === $productType) {
                    $row = [
                        'ICON' => '',
            ...
            if (isset($allowedProductTypes[Catalog\ProductTable::TYPE_SERVICE])) {
                ...
                if (
                    ...
                    (
                        Catalog\ProductTable::TYPE_SERVICE === $productType
                        && null === $currentTab
                    )
        ...
        // group
        if ($boolFeatureSet && self::TAB_GROUP !== $currentTab) {
            if (
                Catalog\ProductTable::TYPE_EMPTY_SKU !== $productType
                && Catalog\ProductTable::TYPE_FREE_OFFER !== $productType
            ) {
                if (!empty($arItems)) {
                    $arItems[] = ['SEPARATOR' => 'Y'];
                }
        ...
        if ($intID > 0) {
            ...
            if (CCatalogSku::TYPE_FULL === $arCatalog['CATALOG_TYPE']) {
                if (
                    ...
                    if ('Y' !== Option::get('catalog', 'show_catalog_tab_with_offers')) {
                        $arResult[self::TAB_CATALOG] = false;
                    }
                } else {
                    if (Catalog\ProductTable::TYPE_EMPTY_SKU !== $productType) {
            ...
            if (null !== $currentTab) {
                ...
                if (self::TAB_GROUP === $currentTab || self::TAB_SET === $currentTab) {
                    $arResult[self::TAB_CATALOG] = true;
                }
                if (
                    self::TAB_SKU === $currentTab
                    && 'Y' === $arCatalog['CATALOG']
                    && 'Y' === Option::get('catalog', 'show_catalog_tab_with_offers')
                ) {
                    $arResult[self::TAB_CATALOG] = true;
                }
                if (self::TAB_SERVICE === $currentTab) {
    ...
    public static function getFormProductTypeName(int $id): ?string
    {
        ...
        if (Catalog\ProductTable::TYPE_PRODUCT === $result) {
            $result = null;
        }
        if (null !== $result) {
    ...
    public static function setCatalogPanelButtons(&$buttons, $iblock, $catalogButtons, $params, $windowParams)
    {
        ...
        if (empty($windowParams) || !is_array($windowParams)) {
            $windowParams = ['width' => 700, 'height' => 400, 'resize' => false];
        }
    ...
    public static function getClearedGridFields(array $options = []): array
    {
        ...
        $useCatalogTab = 'Y' === Option::get('catalog', 'show_catalog_tab_with_offers');
    ...
    public static function getLockedGridFields(array $options = []): array
    {
        ...
        $showCatalog = 'Y' === Option::get('catalog', 'show_catalog_tab_with_offers');
        $useNewCard = (bool) ($options['USE_NEW_CARD'] ?? false);
...
{
    public const NEW_ITEM_COUNT = 3;
    public const PREFIX_SET = 'PRODUCT_SET';
    public const PREFIX_GROUP = 'PRODUCT_GROUP';

    protected static $strMainPrefix = '';
    protected static $intTypeID = 0;
    protected static $arErrors = [];
    protected static $arSrcValues = [];
    protected static $arCheckValues = [];
        ...
        foreach ($arSets as $key => $arOneSet) {
            ...
            Main\Type\Collection::sortByColumn(
                ...
                [
                    ...
                    'ITEM_ID' => [SORT_NUMERIC, SORT_ASC],
                ],
                [
                    'NEW_ITEM' => static function ($value) {
                        return null !== $value;
                    },
                    'EMPTY_ITEM' => static function ($value) {
                        return null !== $value;
                    },
                ],
                null,
...
if (!window.ob<?php echo $blockName; ?>)
{
	window.ob<?php echo $blockName; ?> = new JCCatTblEditExt(<?php echo CUtil::PhpToJSObject($arJSParams); ?>);
}
</script>
			<?php
            unset($blockName);
    ...
    public static function showItemRow($arRow, $arParams)
    {
        ...
        $measure = '';
        if (isset($arRow['RATIO'], $arRow['MEASURE'])) {
    ...
    public static function checkFormValues($arItem)
    {
        ...
        self::$arSrcValues[self::$strMainPrefix] = [];
        self::$arCheckValues[self::$strMainPrefix] = [];
    ...
    public static function saveFormValues($arItem)
    {
        ...
        if (!empty(self::$arCheckValues[self::$strMainPrefix])) {
            foreach (self::$arCheckValues[self::$strMainPrefix] as $key => $arSaveSet) {
                ...
                $boolNew = (0 >= (int) $key);
                if ($boolNew) {
                    if (!isset($arSaveSet['DEL']) || 'Y' !== $arSaveSet['DEL']) {
