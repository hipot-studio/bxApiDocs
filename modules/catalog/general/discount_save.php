...
class discount_save
{
    ...
    protected static $discountFilterCache = [];
    protected static $discountResultCache = [];
     ...
     *
     * @param mixed $intID
     * @param mixed $arGroups
     */
    protected static function __AddGroupsCache($intID, $arGroups = [])
    {
        return true;
    }
     ...
     * @param mixed $arGroups
     */
    protected static function __UpdateGroupsCache($intID, $arGroups = [])
    {
        return true;
    }
    ...
    protected static function __SaleOrderSumm($arOrderFilter, $strCurrency)
    {
        ...
        foreach (GetModuleEvents('catalog', 'OnSaleOrderSumm', true) as $arEvent) {
            $mxOrderCount = ExecuteModuleEventEx($arEvent, [$arOrderFilter]);
        ...
        foreach (GetModuleEvents('catalog', 'OnSaleOrderSummResult', true) as $arEvent) {
            ExecuteModuleEventEx($arEvent, [&$arOrderSumm]);
        }
    ...
    {
        if ('D' === $strType) {
            $arTimeStamp = ['DD' => ($boolDir ? $intSize : -$intSize)];
        } elseif ('M' === $strType) {
            $arTimeStamp = ['MM' => ($boolDir ? $intSize : -$intSize)];
        } elseif ('Y' === $strType) {
            $arTimeStamp = ['YYYY' => ($boolDir ? $intSize : -$intSize)];
        } else {
            $arTimeStamp = [];
        }
    ...
    public static function CheckFields($strAction, &$arFields, $intID = 0)
    {
        ...
        if ((is_set($arFields, 'SITE_ID') || 'ADD' === $strAction) && empty($arFields['SITE_ID'])) {
            $arMsg[] = ['id' => 'SITE_ID', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_EMPTY_SITE')];
            $boolResult = false;
        } else {
            $rsSites = CSite::GetByID($arFields['SITE_ID']);
            if (!$arSite = $rsSites->Fetch()) {
                $arMsg[] = ['id' => 'SITE_ID', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_BAD_SITE')];
                $boolResult = false;
            }
        }

        if ((is_set($arFields, 'NAME') || 'ADD' === $strAction) && ('' === trim($arFields['NAME']))) {
            $arMsg[] = ['id' => 'NAME', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_EMPTY_NAME')];
            $boolResult = false;
        }
        if ((is_set($arFields, 'ACTIVE') || 'ADD' === $strAction) && 'N' !== $arFields['ACTIVE']) {
            $arFields['ACTIVE'] = 'Y';
        }
        if ((is_set($arFields, 'SORT') || 'ADD' === $strAction) && (int) $arFields['SORT'] <= 0) {
            $arFields['SORT'] = 500;
        }
        if ((is_set($arFields, 'CURRENCY') || 'ADD' === $strAction) && empty($arFields['CURRENCY'])) {
            $arMsg[] = ['id' => 'CURRENCY', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_EMPTY_CURRENCY')];
            $boolResult = false;
        }
        if ((is_set($arFields, 'ACTIVE_FROM') || 'ADD' === $strAction) && (!$DB->IsDate($arFields['ACTIVE_FROM'], false, LANGUAGE_ID, 'FULL'))) {
            $arFields['ACTIVE_FROM'] = false;
        }
        if ((is_set($arFields, 'ACTIVE_TO') || 'ADD' === $strAction) && (!$DB->IsDate($arFields['ACTIVE_TO'], false, LANGUAGE_ID, 'FULL'))) {
            $arFields['ACTIVE_TO'] = false;
        }

        if ((is_set($arFields, 'COUNT_SIZE') || 'ADD' === $strAction) && (int) $arFields['COUNT_SIZE'] < 0) {
            $arFields['COUNT_SIZE'] = 0;
        }
        if ((is_set($arFields, 'COUNT_TYPE') || 'ADD' === $strAction) && !in_array($arFields['COUNT_TYPE'], ['D', 'M', 'Y'], true)) {
            $arFields['COUNT_TYPE'] = 'Y';
        }
        if ((is_set($arFields, 'COUNT_FROM') || 'ADD' === $strAction) && (!$DB->IsDate($arFields['COUNT_FROM'], false, LANGUAGE_ID, 'FULL'))) {
            $arFields['COUNT_FROM'] = false;
        }
        if ((is_set($arFields, 'COUNT_TO') || 'ADD' === $strAction) && (!$DB->IsDate($arFields['COUNT_TO'], false, LANGUAGE_ID, 'FULL'))) {
        ...
        if (is_set($arFields, 'COUNT_SIZE') && (int) $arFields['COUNT_SIZE'] > 0) {
        ...
        if ((is_set($arFields, 'ACTION_SIZE') || 'ADD' === $strAction) && (int) $arFields['ACTION_SIZE'] < 0) {
            $arFields['ACTION_SIZE'] = 0;
        }
        if ((is_set($arFields, 'ACTION_TYPE') || 'ADD' === $strAction) && !in_array($arFields['ACTION_TYPE'], ['D', 'M', 'Y'], true)) {
        ...
        if (is_set($arFields, 'RANGES') || 'ADD' === $strAction) {
            if (!is_array($arFields['RANGES']) || empty($arFields['RANGES'])) {
                $arMsg[] = ['id' => 'RANGES', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_EMPTY_RANGES')];
                $boolResult = false;
            } else {
                $boolRepeat = false;
                $arRangeList = [];
                foreach ($arFields['RANGES'] as &$arRange) {
                    if (!is_array($arRange) || empty($arRange)) {
                        $arMsg[] = ['id' => 'RANGES', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_BAD_RANGE')];
                        $boolResult = false;
                    } else {
                        if (empty($arRange['TYPE']) || self::TYPE_FIX !== $arRange['TYPE']) {
                            $arRange['TYPE'] = self::TYPE_PERCENT;
                        }
                        if (isset($arRange['VALUE'])) {
                            $arRange['VALUE'] = str_replace(',', '.', $arRange['VALUE']);
                            $arRange['VALUE'] = (float) $arRange['VALUE'];
                            if (!(0 < $arRange['VALUE'])) {
                                $arMsg[] = ['id' => 'RANGES', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_BAD_RANGE_VALUE')];
                                $boolResult = false;
                        ...
                        if (isset($arRange['RANGE_FROM'])) {
                            ...
                            $arRange['RANGE_FROM'] = (float) $arRange['RANGE_FROM'];
                            if (0 > $arRange['RANGE_FROM']) {
                                $arMsg[] = ['id' => 'RANGES', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_BAD_RANGE_FROM')];
                                $boolResult = false;
                            } else {
                                if (in_array($arRange['RANGE_FROM'], $arRangeList, true)) {
                                    $boolRepeat = true;
                                } else {
                ...
                if ($boolRepeat) {
                    $arMsg[] = ['id' => 'RANGES', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_DUP_RANGE_FROM')];
                    $boolResult = false;
                }
            }
        }
        if (isset($arFields['GROUP_IDS']) || 'ADD' === $strAction) {
            if (!empty($arFields['GROUP_IDS'])) {
                if (!is_array($arFields['GROUP_IDS'])) {
                    $arFields['GROUP_IDS'] = [$arFields['GROUP_IDS']];
                }
                $arValid = [];
            ...
            if (empty($arFields['GROUP_IDS'])) {
                $arMsg[] = ['id' => 'GROUP_IDS', 'text' => Loc::getMessage('BT_MOD_CAT_DSC_SV_ERR_EMPTY_GROUP_IDS')];
                $boolResult = false;
            }
        }
        if ($boolResult) {
            $cond = new CCatalogCondTree();
            $boolCond = $cond->Init(BT_COND_MODE_GENERATE, BT_COND_BUILD_CATALOG, []);
            if (!$boolCond) {
                $boolResult = false;
            } else {
                $arFields['CONDITIONS'] = $cond->GetDefaultConditions();
                $arFields['UNPACK'] = $cond->Generate($arFields['CONDITIONS'], []);
    ...
    public static function GetByID($intID)
    {
        ...
        return CCatalogDiscountSave::GetList([], ['ID' => $intID], false, false, []);
    }
    ...
    public static function GetArrayByID($intID)
    {
        ...
        $rsDiscounts = CCatalogDiscountSave::GetList([], ['ID' => $intID], false, false, []);
    ...
    public static function ChangeActive($intID, $boolActive = true)
    {
        ...
        return CCatalogDiscountSave::Update($intID, ['ACTIVE' => (true === $boolActive ? 'Y' : 'N')], false);
    }

    public static function UserDiscountCalc($intID, $arFields = [], $boolNew = false) {}

    public static function GetDiscount($arParams = [], $getAll = false)
    {
        global $DB, $USER;
        ...
        $key = array_search(2, $arUserGroups, true);
        if (false !== $key) {
        ...
        if (!empty($arCurrentDiscountID)) {
            ...
            if (!isset(self::$discountResultCache[$cacheKey])) {
                ...
                CTimeZone::Disable();
                $rsDiscSaves = CCatalogDiscountSave::GetList([], $arFilter);
                CTimeZone::Enable();
                ...
                unset($arDiscSave, $rsDiscSaves, $intCurrentTime);
            }
            ...
            foreach ($discountList as $arDiscSave) {
                ...
                } elseif ($arDiscSave['ACTION_SIZE'] > 0 && in_array($arDiscSave['ACTION_TYPE'], ['D', 'M', 'Y'], true)) {
                ...
                } elseif ($arDiscSave['COUNT_SIZE'] > 0 && in_array($arDiscSave['COUNT_TYPE'], ['D', 'M', 'Y'], true)) {
                    $strCountPeriod = self::COUNT_TIME_PERIOD;
                    $arCountPeriodBack = CCatalogDiscountSave::__GetTimeStampArray($arDiscSave['COUNT_SIZE'], $arDiscSave['COUNT_TYPE']);
                }

                if (self::COUNT_TIME_INTERVAL === $strCountPeriod) {
                    ...
                    if (false !== $arDiscSave['COUNT_TO_UT']) {
                        if (self::ACTION_TIME_PERIOD === $strActionPeriod && ($arDiscSave['COUNT_TO_UT'] < AddToTimeStamp($arActionPeriodBack, $intCountTime))) {
                            continue;
                        }
                    }
                }

                if (self::ACTION_TIME_PERIOD === $strActionPeriod) {
                    if (self::COUNT_TIME_PERIOD === $strCountPeriod) {
                ...
                } elseif (self::COUNT_TIME_PERIOD === $strCountPeriod) {
                ...
                $rsRanges = CCatalogDiscountSave::GetRangeByDiscount(
                    ['RANGE_FROM' => 'DESC'],
                    ['DISCOUNT_ID' => $arDiscSave['ID'], '<=RANGE_FROM' => $arOrderSumm['RANGE_SUMM']],
                    false,
                    ['nTopCount' => 1]
                );
                ...
                if (!empty($arRange) || $getAll) {
                    ...
                    unset($arDiscSave['ACTIVE_FROM_UT'], $arDiscSave['ACTIVE_TO_UT'], $arDiscSave['COUNT_FROM_UT'], $arDiscSave['COUNT_TO_UT'], $arDiscSave['MICROTIME']);
