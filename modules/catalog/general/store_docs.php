...
class store_docs
{
    ...
    public static function update($id, $arFields)
    {
        ...
        $isConducted = 'Y' === $oldFields['STATUS'];
        $isStatusChangingToUnconducted = isset($arFields['STATUS']) && 'N' === $arFields['STATUS'];
        ...
        if (!empty($strUpdate)) {
            ...
            foreach (GetModuleEvents('catalog', 'OnDocumentUpdate', true) as $arEvent) {
                ExecuteModuleEventEx($arEvent, [$id, $arFields, $oldFields]);
            }
        }
    ...
    public static function conductDocument($documentId, $userId = 0)
    {
        ...
        if ($docType = $docTypes->Fetch()) {
            if (self::CONDUCTED !== $docType['STATUS']) {
    ...
    public static function OnIBlockElementDelete($productID)
    {
        ...
        if ($productID > 0) {
            $dbDeleteElements = CCatalogStoreDocsElement::getList([], ['ELEMENT_ID' => $productID], false, false, ['ID']);
    ...
    protected static function checkFields($action, &$arFields)
    {
        ...
        if ((('ADD' === $action) || isset($arFields['DOC_TYPE'])) && '' === $arFields['DOC_TYPE'] && !isset(self::$types[$arFields['DOC_TYPE']])) {
            $APPLICATION->ThrowException(GetMessage('CAT_DOC_WRONG_TYPE'));

            return false;
        }
        if ((('ADD' === $action) || isset($arFields['SITE_ID'])) && '' === $arFields['SITE_ID']) {
            $APPLICATION->ThrowException(GetMessage('CAT_DOC_WRONG_SITE_ID'));

            return false;
        }
        if ((('ADD' === $action) || isset($arFields['RESPONSIBLE_ID'])) && '' === $arFields['RESPONSIBLE_ID']) {
            $APPLICATION->ThrowException(GetMessage('CAT_DOC_WRONG_RESPONSIBLE'));

            return false;
        }
        if ('ADD' === $action || array_key_exists('STATUS', $arFields)) {
