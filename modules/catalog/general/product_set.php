...
abstract class product_set
{
    ...
    public static function deleteAllSetsByProduct($intProductID, $intSetType)
    {
        ...
        foreach (GetModuleEvents('catalog', 'OnBeforeProductAllSetsDelete', true) as $arEvent) {
            if (false === ExecuteModuleEventEx($arEvent, [$intProductID, $intSetType])) {
                return false;
            }
        }
        ...
        switch ($intSetType) {
            ...
            case self::TYPE_GROUP:
                if (!static::isProductHaveSet($intProductID, self::TYPE_GROUP)) {
                    CCatalogProduct::Update($intProductID, ['BUNDLE' => Catalog\ProductTable::STATUS_NO]);
                }
        ...
        foreach (GetModuleEvents('catalog', 'OnProductAllSetsDelete', true) as $arEvent) {
            ExecuteModuleEventEx($arEvent, [$intProductID, $intSetType]);
        }
    ...
    protected static function checkFieldsToAdd(&$arFields, $boolCheckNew = false)
    {
        ...
        $boolCheckNew = (bool) $boolCheckNew;
        ...
        if (empty(self::$arErrors)) {
            ...
            if (!$boolCheckNew) {
                if (0 >= $arFields['ITEM_ID']) {
                    self::$arErrors[] = ['id' => 'ITEM_ID', 'text' => Loc::getMessage('BT_CAT_SET_ERR_PRODUCT_ID_IS_BAD')];
                }
            } else {
                if (0 > $arFields['ITEM_ID']) {
                    self::$arErrors[] = ['id' => 'ITEM_ID', 'text' => Loc::getMessage('BT_CAT_SET_ERR_PRODUCT_ID_IS_BAD')];
            ...
            if (self::TYPE_SET !== $arFields['TYPE'] && self::TYPE_GROUP !== $arFields['TYPE']) {
                self::$arErrors[] = ['id' => 'TYPE', 'text' => Loc::getMessage('BT_CAT_SET_ERR_TYPE_IS_BAD')];
            }

            if (empty($arFields['ITEMS']) || !is_array($arFields['ITEMS'])) {
                self::$arErrors[] = ['id' => 'ITEMS', 'text' => Loc::getMessage('BT_CAT_SET_ERR_ITEMS_IS_ABSENT')];
            }
        }
        ...
        if (empty(self::$arErrors)) {
            ...
            foreach ($arFields['ITEMS'] as &$arOneItem) {
                ...
                if (isset($arProductInSet[$arOneItem['ITEM_ID']])) {
                    self::$arErrors[] = ['id' => 'ITEM_ID', 'text' => Loc::getMessage('BT_CAT_PRODUCT_SET_ERR_ITEM_ID_DUBLICATE')];

                    continue;
                }
                $arProductInSet[$arOneItem['ITEM_ID']] = true;
                $arOneItem['QUANTITY'] = (float) $arOneItem['QUANTITY'];
                ...
                if (self::TYPE_SET === $arFields['TYPE']) {
                    ...
                    if (false !== $arOneItem['DISCOUNT_PERCENT']) {
                        $arOneItem['DISCOUNT_PERCENT'] = (float) $arOneItem['DISCOUNT_PERCENT'];
                        if (0 > $arOneItem['DISCOUNT_PERCENT'] || 100 < $arOneItem['DISCOUNT_PERCENT']) {
                            self::$arErrors[] = ['id' => 'DISCOUNT_PERCENT', 'text' => Loc::getMessage('BT_CAT_PRODUCT_SET_ERR_DISCOUNT_PERCENT_IS_BAD')];
            ...
            if (empty($arValidItems)) {
                self::$arErrors[] = ['id' => 'ITEMS', 'text' => Loc::getMessage('BT_CAT_SET_ERR_EMPTY_VALID_ITEMS')];
            } else {
                $arFields['ITEMS'] = $arValidItems;
            }
            unset($arValidItems);
            if (100 < $dblDiscountPercent) {
                self::$arErrors[] = ['id' => 'DISCOUNT_PERCENT', 'text' => Loc::getMessage('BT_CAT_PRODUCT_SET_ERR_ALL_DISCOUNT_PERCENT_IS_BAD')];
            }
        }
        ...
        if (empty(self::$arErrors)) {
            ...
            if (!self::$disableCheckProduct) {
                if (self::TYPE_GROUP === $arFields['TYPE']) {
    ...
    protected static function checkFieldsToUpdate($intID, &$arFields)
    {
        ...
        $intID = (int) $intID;
        if ($intID <= 0) {
            self::$arErrors[] = ['id' => 'ID', 'text' => Loc::getMessage('BT_CAT_SET_ERR_ID_IS_BAD')];
        }
                ...
                if (!self::$disableCheckProduct) {
                    if (!static::checkProducts($arCurrent['TYPE'], array_keys($arProductInSet))) {
                        self::$arErrors[] = [
                            'id' => 'ITEMS',
                            'text' => (
                                self::TYPE_GROUP === $arCurrent['TYPE']
                                ? Loc::getMessage('BT_CAT_PRODUCT_SET_ERR_BAD_ITEMS_IN_GROUP')
                                : Loc::getMessage('BT_CAT_PRODUCT_SET_ERR_BAD_ITEMS_IN_SET')
                            ),
                        ];
    ...
    {
        return
            isset($item['QUANTITY_TRACE']) && 'Y' === $item['QUANTITY_TRACE']
            && isset($item['CAN_BUY_ZERO']) && 'N' === $item['CAN_BUY_ZERO'];
    }

    protected static function checkProducts($type, array $productList)
    {
        $type = (int) $type;
        if (self::TYPE_SET !== $type && self::TYPE_GROUP !== $type) {
            return false;
        }
        ...
        while ($product = $productIterator->fetch()) {
            ...
            if (isset($allowTypes[$productType], $productList[$id])) {
                unset($productList[$id], $productType, $id);
            }
        }
