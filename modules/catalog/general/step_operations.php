...
class CCatalogStepOperations
{
    ...
    public function getMessage()
    {
        $messageParams = [
            ...
            'TYPE' => 'PROGRESS',
            'DETAILS' => str_replace(['#ALL#', '#COUNT#'], [$this->allCounter, $this->allOperationCounter], $this->progressTemplate),
            'HTML' => true,
        ];
    ...
    {
        $this->finishOperation = (true === $finish);
    }
...
{
    public const SESSION_PREFIX = 'PSA';
    ...
    public function runOperation()
    {
        ...
        $emptyList = true;
        CTimeZone::Disable();
        $filter = ['TYPE' => CCatalogProductSet::TYPE_SET, 'SET_ID' => 0];
        if ($this->lastID > 0) {
            $filter['>ID'] = $this->lastID;
        }
        $topCount = ($this->maxOperationCounter > 0 ? ['nTopCount' => $this->maxOperationCounter] : false);
        $productSetsIterator = CCatalogProductSet::getList(
            ['ID' => 'ASC'],
            $filter,
            false,
            $topCount,
            ['ID', 'OWNER_ID', 'ITEM_ID', 'MODIFIED_BY', 'TIMESTAMP_X']
        );
        while ($productSet = $productSetsIterator->Fetch()) {
            $emptyList = false;
            $productSet['MODIFIED_BY'] = (int) $productSet['MODIFIED_BY'];
            if (0 === $productSet['MODIFIED_BY']) {
    ...
    public static function getAllCounter()
    {
        return (int) CCatalogProductSet::getList(
            [],
            ['TYPE' => CCatalogProductSet::TYPE_SET, 'SET_ID' => 0],
            []
        );
    }
}
...
{
    public const SESSION_PREFIX = 'PA';
    ...
    private $existPriceIds = [];
    private $existIdsByType = [];
    private $measureRatios = [];
    ...
    public function runOperation()
    {
        ...
        $this->currentList = [];
        $this->currentIdsList = [];
        $productIterator = $this->getProductIterator([], []);
    ...
    {
        $this->config = [
            'SEPARATE_SKU_MODE' => 'Y' === (string) Main\Config\Option::get('catalog', 'show_catalog_tab_with_offers'),
    ...
    private function loadSkuData()
    {
        ...
        foreach ($this->currentIdsList as $id) {
            ...
            foreach ($offers[$id] as $offerId => $row) {
                $allOffers[] = $offerId;
                if ('Y' !== $row['ACTIVE'] || 'Y' !== $row['AVAILABLE']) {
                    continue;
                }
            ...
            if ($this->config['CHECK_SKU_PRICES'] && !$this->config['SEPARATE_SKU_MODE']) {
                if (Catalog\Product\Sku::OFFERS_AVAILABLE === $this->currentList[$id]['SKU_STATE']) {
    ...
    private function loadProductSets()
    {
        ...
        while ($row = $iterator->fetch()) {
            $id = (int) $row['OWNER_ID'];
            if (CCatalogProductSet::TYPE_SET === $row['TYPE']) {
                $this->currentList[$id]['SET_EXISTS'] = true;
            }
            if (CCatalogProductSet::TYPE_GROUP === $row['TYPE']) {
                $this->currentList[$id]['BUNDLE_EXISTS'] = true;
            }
        }
        unset($row, $iterator, $tableName, $helper, $conn);
    }
    ...
    private function loadMeasureRatios()
    {
        ...
        foreach (array_chunk($this->currentIdsList, 500) as $pageIds) {
            $iterator = Catalog\MeasureRatioTable::getList([
                'select' => ['*'],
                'filter' => ['@PRODUCT_ID' => $pageIds],
                'order' => ['PRODUCT_ID' => 'ASC', 'RATIO' => 'ASC'],
            ]);
            while ($row = $iterator->fetch()) {
                $productId = (int) $row['PRODUCT_ID'];
    ...
    private function updateProductData()
    {
        ...
        $checkMeasure = (
            ...
            && CCatalogSku::TYPE_PRODUCT !== $this->iblockData['CATALOG_TYPE']
            && (isset($this->iblockData['SUBSCRIPTION']) && 'Y' !== $this->iblockData['SUBSCRIPTION'])
        );
        ...
        foreach ($this->currentIdsList as $id) {
            ...
            if ($this->config['CHECK_AVAILABLE']) {
                ...
                if ($this->config['CHECK_SETS']) {
                    $fields['BUNDLE'] = (
                        $product['BUNDLE_EXISTS']
                        ? Catalog\ProductTable::STATUS_YES
                        : Catalog\ProductTable::STATUS_NO
                    );
                }
                if ($checkMeasure) {
                    if (Catalog\ProductTable::TYPE_SET === $fields['TYPE']) {
                        if (null !== $this->measureIds['BASE']) {
                            $fields['MEASURE'] = $this->measureIds['BASE'];
                        }
                    } else {
                        if ((int) $product['MEASURE'] <= 0 && null !== $this->measureIds['DEFAULT']) {
                ...
                if ($productResult->isSuccess()) {
                    ...
                    if (
                        Catalog\ProductTable::TYPE_OFFER === $product['TYPE']
                        || Catalog\ProductTable::TYPE_FREE_OFFER === $product['TYPE']
                    ) {
            ...
            if ($product['SUCCESS']) {
                ...
                if ($this->config['CHECK_SKU_PRICES']) {
                    ...
                    if (Catalog\ProductTable::TYPE_SKU === $product['TYPE']) {
    ...
    private function updateProductPrices($id, array $product)
    {
        ...
        if (Catalog\ProductTable::TYPE_SKU === $product['TYPE'] && !$this->config['SEPARATE_SKU_MODE']) {
            return;
        }
        ...
        if (Catalog\ProductTable::TYPE_EMPTY_SKU === $product['TYPE']) {
            unset($this->prices[$id], $this->existIdsByType[$id], $this->existPriceIds[$id]);
        ...
        foreach (array_keys($this->prices[$id]) as $rowId) {
            ...
            if (
                ...
                || 0 === $this->currencyReference[$row['CURRENCY']]
            ) {
                continue;
            }
        ...
        unset($rowResult, $newScale, $baseRate, $row, $rowId, $this->prices[$id]);
        ...
        if (!empty($this->calculatePrices[$id])) {
            foreach (array_keys($this->calculatePrices[$id]) as $resultPriceType) {
                ...
                if (!empty($this->existIdsByType[$id][$resultPriceType])) {
                    ...
                    unset($this->existPriceIds[$id][$rowId], $this->prices[$id][$rowId]);
                }
                if (null === $rowId) {
                    $row['PRODUCT_ID'] = $id;
    ...
    private function updateMeasureRatios($id, array $product)
    {
        ...
        $action = '';
        if (isset($this->iblockData['SUBSCRIPTION']) && 'Y' === $this->iblockData['SUBSCRIPTION']) {
            ...
            Catalog\ProductTable::TYPE_PRODUCT === $product['TYPE']
            || Catalog\ProductTable::TYPE_OFFER === $product['TYPE']
            || Catalog\ProductTable::TYPE_FREE_OFFER === $product['TYPE']
        ) {
            if (!empty($this->measureRatios[$id]['RATIOS'])) {
        ...
        } elseif (Catalog\ProductTable::TYPE_SET === $product['TYPE']) {
            if (!empty($this->measureRatios[$id]['RATIOS'])) {
                $action = 'set';
            } else {
                $action = 'create';
            }
        } elseif (Catalog\ProductTable::TYPE_EMPTY_SKU === $product['TYPE']) {
            if (!empty($this->measureRatios[$id]['RATIOS'])) {
                $action = 'remove';
            }
        } elseif (Catalog\ProductTable::TYPE_SKU === $product['TYPE']) {
        ...
        switch ($action) {
            ...
            case 'check':
                if (!$this->measureRatios[$id]['DEFAULT_EXISTS']) {
                    ...
                    $ratioResult = Catalog\MeasureRatioTable::update($firstRatio['ID'], ['IS_DEFAULT' => 'Y']);
                    unset($ratioResult, $firstRatio);
                }
                if (!empty($this->measureRatios[$id]['DOUBLES'])) {
                    foreach ($this->measureRatios[$id]['DOUBLES'] as $ratioId) {
                        $ratioResult = Catalog\MeasureRatioTable::update($ratioId, ['IS_DEFAULT' => 'N']);
                    }
            ...
            case 'set':
                ...
                unset($row);
                if (null === $ratioId && null !== $this->measureRatios[$id]['DEFAULT_RATIO_ID']) {
        ...
        unset($action, $this->measureRatios[$id]);
    }
    ...
    private function prepareSetState(array $fields, array $product): array
    {
        ...
        if (Catalog\ProductTable::TYPE_SET === $fields['TYPE'] && !$product['SET_EXISTS']) {
            $fields['TYPE'] = Catalog\ProductTable::TYPE_PRODUCT;
        } elseif (Catalog\ProductTable::TYPE_PRODUCT === $fields['TYPE'] && $product['SET_EXISTS']) {
...
{
    public const SESSION_PREFIX = 'PS';
    public const SETS_ID = 'SETS';
    ...
    public function runOperation()
    {
        ...
        if (self::SETS_ID === $this->params['IBLOCK_ID']) {
    ...
    public static function getCatalogList()
    {
        ...
        $iblockIterator = Catalog\ProductTable::getList([
            'select' => ['IBLOCK_ID' => 'IBLOCK_ELEMENT.IBLOCK_ID', new Main\Entity\ExpressionField('CNT', 'COUNT(*)')],
            'filter' => [static::getProductFilter()],
            'group' => ['IBLOCK_ID'],
            'order' => ['IBLOCK_ID' => 'ASC'],
        ]);
        ...
        unset($iblock, $iblockIterator, $iblockList);
...
{
    public const NOTIFY_ID = 'CATALOG_REINDEX';
    ...
    protected function runExtendedOperation(array $product)
    {
        ...
        if (
            Catalog\ProductTable::TYPE_EMPTY_SKU === $product['TYPE']
            || (Catalog\ProductTable::TYPE_SKU === $product['TYPE'] && !$this->isSeparateSkuMode())
        ) {
            return;
        }
