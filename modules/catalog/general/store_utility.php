...
class store_utility
{
    ...
    public static function getProductInfo($elementId)
    {
        ...
        $dbProduct = CIBlockElement::GetList(
            ...
            false,
            false,
            ['ID', 'IBLOCK_ID', 'IBLOCK_SECTION_ID', 'DETAIL_PICTURE', 'PREVIEW_PICTURE', 'NAME', 'XML_ID']
        );
        ...
        if ($imgCode > 0) {
            ...
            $arImgProduct = CFile::ResizeImageGet($arFile, ['width' => 80, 'height' => 80], BX_RESIZE_IMAGE_PROPORTIONAL, false, false);
    ...
    {
        $strError = '';
        $numberDisplayedElements = (int) $numberDisplayedElements;
        ...
        if (is_array($arProduct)) {
            foreach ($arProduct as $key => $product) {
                ...
                if ($key >= ($numberDisplayedElements - 1)) {
                    $strError .= "\n...".GetMessage('CAT_DOC_AND_MORE', ['#COUNT#' => (count($arProduct) - $numberDisplayedElements)]);
     ...
     */
    public static function getMultipleFilesFromPost(HttpRequest $request, array $fieldList): ?array
    {
        if (empty($fieldList)) {
            return null;
        }
        ...
        foreach ($fieldList as $fieldId) {
            ...
            if (isset($requestFields[$deleteFieldId]) && is_array($requestFields[$deleteFieldId])) {
                foreach ($requestFields[$deleteFieldId] as $rowId => $value) {
                    ...
                    if (
                        isset($result[$fieldId][$rowId], $result[$fieldId][$rowId]['FILE_ID'])
                    ) {
     ...
     */
    public static function getFilesFromPost(HttpRequest $request, array $fieldList): ?array
    {
        if (empty($fieldList)) {
            return null;
        }
        ...
        foreach ($fieldList as $fieldId) {
            ...
            $deleteFieldId = $fieldId.'_del';
            if (isset($requestFields[$deleteFieldId]) && 'Y' === $requestFields[$deleteFieldId]) {
                if (isset($result[$fieldId], $result[$fieldId]['FILE_ID'])) {
    ...
    public static function isAllowShowShippingCenter(): bool
    {
        ...
        return 'Y' === Option::get('catalog', 'show_store_shipping_center');
    }
}
