...
abstract class CCatalogDocsTypes
{
    ...
    public static function conductDocument($documentId, $userId, $currency, $contractorId)
    {
        ...
        if (null === $document || !static::checkConductDocument($document)) {
            return false;
        }
    ...
    public static function cancellationDocument($documentId, $userId)
    {
        ...
        if (null === $document || !static::checkCancellationDocument($document)) {
            return false;
        }
    ...
    protected static function loadDocument(int $documentId, array $options = []): ?array
    {
        ...
        while ($row = $iterator->fetch()) {
            ...
            if (null !== $row['STORE_TO']) {
                $row['STORE_TO'] = (int) $row['STORE_TO'];
                if ($row['STORE_TO'] <= 0) {
                    $row['STORE_TO'] = null;
                }
            }
            if (null !== $row['STORE_FROM']) {
        ...
        unset($row, $iterator, $productList);
    ...
    protected static function loadProducts(array $document): array
    {
        ...
        foreach (array_chunk($elements, 500) as $pageIds) {
            ...
            while ($row = $iterator->Fetch()) {
                ...
                $productList[$id]['QUANTITY_LIMIT'] = ('Y' === $row['QUANTITY_TRACE'] && 'N' === $row['CAN_BUY_ZERO']);
            }
            unset($row, $iterator);
        ...
        unset($pageIds, $elements, $productList);
    ...
    protected static function checkDocumentProducts(array $document): bool
    {
        ...
        foreach ($document['ELEMENTS'] as $productId => $data) {
            ...
            if (
                Catalog\ProductTable::TYPE_PRODUCT !== $data['TYPE']
                && Catalog\ProductTable::TYPE_OFFER !== $data['TYPE']
            ) {
                $errors[] = Loc::getMessage(
    ...
    protected static function loadBarcodes(array $document): array
    {
        ...
        unset($productId, $row, $iterator, $list, $productList);
    ...
    protected static function applyBarCode($arFields, $userId)
    {
        ...
        $userId = (int) $userId;
        $result = false;
        $rsProps = CCatalogStoreBarCode::GetList([], ['BARCODE' => $barCode], false, false, ['ID', 'STORE_ID', 'PRODUCT_ID']);
        if ($arBarCode = $rsProps->Fetch()) {
            if ($storeFromId > 0) { // deduct or moving
                if ($storeToId > 0) { // moving
                    if ($arBarCode['STORE_ID'] === $storeFromId && $arBarCode['PRODUCT_ID'] === $elementId) {
                        ...
                        $APPLICATION->ThrowException(Loc::getMessage('CATALOG_STORE_DOCS_ERR_WRONG_STORE_BARCODE', ['#STORE#' => '"'.$storeName.'"', '#PRODUCT#' => '"'.$arFields['ELEMENT_NAME'].'"', '#BARCODE#' => '"'.$barCode.'"']));

                        return false;
                    }
                }
            } else {
                $APPLICATION->ThrowException(Loc::getMessage('CATALOG_STORE_DOCS_ERR_BARCODE_ALREADY_EXIST', ['#PRODUCT#' => '"'.$arFields['ELEMENT_NAME'].'"', '#BARCODE#' => '"'.$barCode.'"']));

                return false;
            }
            if ($newStore > 0) {
                $result = CCatalogStoreBarCode::update($arBarCode['ID'], ['STORE_ID' => $storeToId, 'MODIFIED_BY' => $userId]);
            } else {
            ...
            if ($storeToId > 0) {
                $result = CCatalogStoreBarCode::Add(['PRODUCT_ID' => $elementId, 'STORE_ID' => $storeToId, 'BARCODE' => $barCode, 'MODIFIED_BY' => $userId, 'CREATED_BY' => $userId]);
            }
        }
    ...
    protected static function checkAmountField($arDocElement, $name = '')
    {
        global $APPLICATION;
        $name = (string) $name;
        if ((float) $arDocElement['AMOUNT'] <= 0) {
            if ('' === $name) {
                $dbProduct = CIBlockElement::GetList(
                    [],
                    ['ID' => $arDocElement['ELEMENT_ID']],
                    false,
                    false,
                    ['ID', 'NAME']
                );
            ...
            $APPLICATION->ThrowException(Loc::getMessage('CATALOG_STORE_DOCS_ERR_WRONG_AMOUNT', ['#PRODUCT#' => '"'.$name.'"']));
...
class CCatalogArrivalDocs extends CCatalogDocsTypes
{
    ...
    protected static function checkParamsForConduction(int $documentId, int $userId, ?string $currency, int $contractorId): bool
    {
        ...
        if (Manager::getActiveProvider()) {
            ...
            $isContractorSpecified = null !== $contractor;
        } else {
    ...
    protected static function getDocumentActions(string $action, array $document, int $userId): ?array
    {
        ...
        foreach ($elements as $productId => $element) {
            ...
            foreach ($positions as $item) {
                if (self::ACTION_CONDUCTION === $action) {
                ...
                } elseif (self::ACTION_CANCEL === $action) {
