...
class CCatalogSku extends CAllCatalogSku
{
    ...
    protected static $arOfferCache = [];
    protected static $arProductCache = [];
    protected static $arPropertyCache = [];
    protected static $arIBlockCache = [];
    ...
    public static function GetInfoByOfferIBlock($intIBlockID)
    {
        ...
        if (!isset(self::$arOfferCache[$intIBlockID])) {
            ...
            $iblockIterator = Catalog\CatalogIblockTable::getList([
                'select' => ['IBLOCK_ID', 'PRODUCT_IBLOCK_ID', 'SKU_PROPERTY_ID', 'VERSION' => 'IBLOCK.VERSION'],
                'filter' => ['=IBLOCK_ID' => $intIBlockID, '!=PRODUCT_IBLOCK_ID' => 0],
            ]);
    ...
    public static function GetInfoByProductIBlock($intIBlockID)
    {
        ...
        if (!isset(self::$arProductCache[$intIBlockID])) {
            ...
            $iblockIterator = Catalog\CatalogIblockTable::getList([
                'select' => ['IBLOCK_ID', 'PRODUCT_IBLOCK_ID', 'SKU_PROPERTY_ID', 'VERSION' => 'IBLOCK.VERSION'],
                'filter' => ['=PRODUCT_IBLOCK_ID' => $intIBlockID],
            ]);
            $arResult = $iblockIterator->fetch();
            if (!empty($arResult)) {
                $arResult['IBLOCK_ID'] = (int) $arResult['IBLOCK_ID'];
    ...
    public static function GetInfoByIBlock($intIBlockID)
    {
        ...
        if (!isset(self::$arIBlockCache[$intIBlockID])) {
            ...
            $catalogIterator = Catalog\CatalogIblockTable::getList([
                'select' => ['IBLOCK_ID', 'PRODUCT_IBLOCK_ID', 'SKU_PROPERTY_ID', 'VAT_ID', 'YANDEX_EXPORT', 'SUBSCRIPTION'],
                'filter' => [[
                    'LOGIC' => 'OR',
            ...
            unset($boolProductIBlock, $boolIBlock, $boolExists, $arProductIBlock, $arIBlock, $result);
        }
    ...
    public static function getExistOffers($productID, $iblockID = 0)
    {
        ...
        if (0 === $iblockID) {
            ...
            if (!empty($iblockList)) {
                ...
                if (!empty($iblockList)) {
                    $iblockIterator = Catalog\CatalogIblockTable::getList([
                        'select' => ['IBLOCK_ID', 'PRODUCT_IBLOCK_ID', 'SKU_PROPERTY_ID', 'VERSION' => 'IBLOCK.VERSION'],
                        'filter' => ['@PRODUCT_IBLOCK_ID' => array_keys($iblockList)],
                    ]);
        ...
        unset($sku, $iblockProductID, $iblockID, $iblockProduct, $propertyIdField, $helper, $conn);
    ...
    public static function getOffersList(
        ...
        $fields = array_merge($fields, ['ID', 'IBLOCK_ID']);
        ...
        if (!empty($propertyFilter['ID']) || !empty($propertyFilter['CODE'])) {
            sort($offersIblock);
            $propertyIblock = ['@IBLOCK_ID' => $offersIblock];
            if (!empty($propertyFilter['ID'])) {
            ...
            if (!isset($propertyCache[$propertyKey])) {
                $propertyCache[$propertyKey] = array_fill_keys($offersIblock, []);
        ...
        $result = array_fill_keys($productID, []);
            ...
            if (!empty($offersLinks) && $needProperties) {
                ...
                foreach (array_chunk($offerIds, 500) as $pageIds) {
                    ...
                    CIBlockElement::GetPropertyValuesArray(
                        ...
                        $pageOffersFilter,
                        ['ID' => $iblockProperties[$iblockSku[$iblockID]['IBLOCK_ID']]],
                        $options
                    );
    ...
    public static function getProductList($offerID, $iblockID = 0)
    {
        ...
        if (0 === $iblockID) {
            ...
            if (!empty($iblockList)) {
                ...
                if (!empty($iblockList)) {
                    $iblockIterator = Catalog\CatalogIblockTable::getList([
                        'select' => ['IBLOCK_ID', 'PRODUCT_IBLOCK_ID', 'SKU_PROPERTY_ID', 'VERSION' => 'IBLOCK.VERSION'],
                        'filter' => ['@IBLOCK_ID' => array_keys($iblockList), '!=PRODUCT_IBLOCK_ID' => 0],
                    ]);
                    while ($iblock = $iblockIterator->fetch()) {
                        $iblock['IBLOCK_ID'] = (int) $iblock['IBLOCK_ID'];
                        $iblock['PRODUCT_IBLOCK_ID'] = (int) $iblock['PRODUCT_IBLOCK_ID'];
            ...
            if (empty(self::$arProductCache[$iblockID])) {
                if (isset(self::$arOfferCache[$iblockID])) {
                    ...
                    $iblockIterator = Catalog\CatalogIblockTable::getList([
                        'select' => ['IBLOCK_ID', 'PRODUCT_IBLOCK_ID', 'SKU_PROPERTY_ID', 'VERSION' => 'IBLOCK.VERSION'],
                        'filter' => ['=IBLOCK_ID' => $iblockID, '!=PRODUCT_IBLOCK_ID' => 0],
                    ]);
                    if ($iblock = $iblockIterator->fetch()) {
                        $iblock['IBLOCK_ID'] = (int) $iblock['IBLOCK_ID'];
                        $iblock['PRODUCT_IBLOCK_ID'] = (int) $iblock['PRODUCT_IBLOCK_ID'];
        ...
        $result = array_fill_keys($offerID, []);
        ...
        unset($iblockID, $iblockOffers, $helper, $conn);
