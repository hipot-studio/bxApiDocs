...
class price
{
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if ($currencyExist) {
            ...
            if (empty($currency)) {
                $APPLICATION->ThrowException(Loc::getMessage('KGP_NO_CURRENCY', ['#ID#' => $arFields['CURRENCY']]), 'CURRENCY');
        ...
        if (isset($arFields['QUANTITY_FROM']) && false !== $arFields['QUANTITY_FROM']) {
            $arFields['QUANTITY_FROM'] = (int) $arFields['QUANTITY_FROM'];
            if ($arFields['QUANTITY_FROM'] <= 0) {
                $arFields['QUANTITY_FROM'] = false;
            }
        }
        if (isset($arFields['QUANTITY_TO']) && false !== $arFields['QUANTITY_TO']) {
    ...
    public static function GetByID($id)
    {
        ...
        if (isset($priceTypes[$price['CATALOG_GROUP_ID']])) {
            $price['CATALOG_GROUP_NAME'] = (
                null !== $priceTypes[$price['CATALOG_GROUP_ID']]['NAME_LANG']
        ...
        $iterator = Catalog\GroupAccessTable::getList([
            ...
            'filter' => [
                ...
                '@GROUP_ID' => (CCatalog::IsUserExists() ? $USER->GetUserGroupArray() : [2]),
            ],
        ]);
        while ($row = $iterator->fetch()) {
            if (Catalog\GroupAccessTable::ACCESS_BUY === $row['ACCESS']) {
                $price['CAN_ACCESS'] = 'Y';
            } elseif (Catalog\GroupAccessTable::ACCESS_VIEW === $row['ACCESS']) {
                $price['CAN_BUY'] = 'Y';
            }
        }
        unset($row, $iterator);
     ...
     * @param bool $boolExt
     *
     * @return null|array|bool|false|mixed
     */
    public static function GetBasePrice($productID, $quantityFrom = false, $quantityTo = false, $boolExt = true)
    {
        ...
        if (false === $boolExt) {
            $arSelect = ['ID', 'PRODUCT_ID', 'EXTRA_ID', 'CATALOG_GROUP_ID', 'PRICE', 'CURRENCY', 'TIMESTAMP_X',
    ...
    /**
     * @deprecated deprecated since catalog 17.6.0
     * @see Price::add and \Bitrix\Catalog\Model\Price::update
     *
     * @param int       $ProductID
    ...
    public static function SetBasePrice($ProductID, $Price, $Currency, $quantityFrom = false, $quantityTo = false, $bGetID = false)
    {
        ...
        $arFields['QUANTITY_FROM'] = (false === $quantityFrom ? false : (int) $quantityFrom);
        $arFields['QUANTITY_TO'] = (false === $quantityTo ? false : (int) $quantityTo);
        ...
        if ('EXTRA' === $TYPE) {
            ...
            while ($res = $db_res->Fetch()) {
                $parentFilter = [
                    ...
                    'QUANTITY_FROM' => (null === $res['QUANTITY_FROM'] ? false : $res['QUANTITY_FROM']),
                    'QUANTITY_TO' => (null === $res['QUANTITY_TO'] ? false : $res['QUANTITY_TO']),
                ];
    ...
    public static function DeleteByProduct($ProductID, $arExceptionIDs = [])
    {
        global $DB;
        ...
        foreach (GetModuleEvents('catalog', 'OnBeforeProductPriceDelete', true) as $arEvent) {
            if (false === ExecuteModuleEventEx($arEvent, [$ProductID, &$arExceptionIDs])) {
                return false;
            }
        }
        ...
        foreach (GetModuleEvents('catalog', 'OnProductPriceDelete', true) as $arEvent) {
            ExecuteModuleEventEx($arEvent, [$ProductID, $arExceptionIDs]);
        }

        Catalog\Product\Sku::calculatePrice($ProductID, null, null, []);
    ...
    public static function ReCountForBase(&$arFields)
    {
        ...
        $arFilter = ['PRODUCT_ID' => $arFields['PRODUCT_ID'], '!CATALOG_GROUP_ID' => $arFields['CATALOG_GROUP_ID']];
        if (isset($arFields['QUANTITY_FROM'])) {
    ...
    public static function ReCountFromBase(&$arFields, &$boolBase)
    {
        ...
        if (!empty($arBaseGroup)) {
            if ($arFields['CATALOG_GROUP_ID'] === $arBaseGroup['ID']) {
                ...
                if (!empty($arFields['EXTRA_ID']) && (int) $arFields['EXTRA_ID'] > 0) {
                    $arExtra = CExtra::GetByID($arFields['EXTRA_ID']);
                    if (!empty($arExtra)) {
                        $arExtra['PERCENTAGE'] = (float) $arExtra['PERCENTAGE'];
                        $arFilter = ['PRODUCT_ID' => $arFields['PRODUCT_ID'], 'CATALOG_GROUP_ID' => $arBaseGroup['ID']];
                        if (isset($arFields['QUANTITY_FROM'])) {
                            $arFilter['QUANTITY_FROM'] = $arFields['QUANTITY_FROM'];
                        }
                        if (isset($arFields['QUANTITY_TO'])) {
    ...
    private static function normalizeFields(array &$fields)
    {
        if (isset($fields['QUANTITY_FROM'])) {
            ...
            } elseif (false === $fields['QUANTITY_FROM'] || 0 === $fields['QUANTITY_FROM']) {
                $fields['QUANTITY_FROM'] = null;
            }
        }
        if (isset($fields['QUANTITY_TO'])) {
            ...
            } elseif (false === $fields['QUANTITY_TO'] || 0 === $fields['QUANTITY_TO']) {
                $fields['QUANTITY_TO'] = null;
            }
        }
