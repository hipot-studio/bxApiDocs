...
class product_group
{
    ...
    {
        if ((is_set($arFields, 'PRODUCT_ID') || 'ADD' === $ACTION) && (int) $arFields['PRODUCT_ID'] <= 0) {
            return false;
        }
        ...
        if (is_set($arFields, 'ACCESS_LENGTH') || 'ADD' === $ACTION) {
            $arFields['ACCESS_LENGTH'] = (int) $arFields['ACCESS_LENGTH'];
            if ($arFields['ACCESS_LENGTH'] < 0) {
                $arFields['ACCESS_LENGTH'] = 0;
            }
        }

        if ((is_set($arFields, 'ACCESS_LENGTH_TYPE') || 'ADD' === $ACTION) && !array_key_exists($arFields['ACCESS_LENGTH_TYPE'], CCatalogProduct::GetTimePeriodTypes(true))) {
