...
class CCatalogActionCtrlBasketProductFields extends CCatalogCondCtrlIBlockFields
{
    ...
    {
        $key = array_search(CSaleActionCtrlAction::GetControlID(), $arControls, true);
        if (false !== $key) {
...
class CCatalogGifterProduct extends CGlobalCondCtrlAtoms
{
    ...
    public static function GetControls($strControlID = false)
    {
        ...
        if (isset($arControlList[$strControlID])) {
    ...
    public static function GenerateApplyCallableFilter($controlId, array $gifts, $type)
    {
        ...
        return static function ($row) use ($controlId, $gifts, $type) {
            static $isApplied = false;
            if ($isApplied && CSaleDiscountActionApply::GIFT_SELECT_TYPE_ONE === $type) {
                return false;
            }
            $right = false;
                ...
                case 'GifterCondIBElement':
                    $right =
                        (
                            ...
                            || (isset($row['CATALOG']['ID'], $gifts[$row['CATALOG']['ID']]))
                        )
