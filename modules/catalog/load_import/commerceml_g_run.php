...
function cmlStartElement($parser, $name, $attrs)
{
    ...
    switch ($name) {
        case $nameUTF['Catalog']:
            ...
            if (in_array($nameUTF['ID'], array_keys($attrs), true)) {
                $currentCatalog['ID'] = $attrs[$nameUTF['ID']];
            }
            if (in_array($nameUTF['Name'], array_keys($attrs), true)) {
                $currentCatalog['Name'] = $attrs[$nameUTF['Name']];
            }
            if (in_array($nameUTF['Description'], array_keys($attrs), true)) {
            ...
            $dbIBlockList = CIBlock::GetList(
                [],
                ['=TYPE' => $IBLOCK_TYPE_ID, '=XML_ID' => $currentCatalog['ID'], 'MIN_PERMISSION' => 'W']
            );
            if ($arIBlock = $dbIBlockList->Fetch()) {
                $arIBlock['VERSION'] = (int) $arIBlock['VERSION'];
            ...
            if (!$res) {
                ...
                $arIBlockCache[$currentCatalog['ID']] = (int) $currentCatalog['BID'];
                $iBlockIDString .= ','.(int) $currentCatalog['BID'];
                if (!CCatalog::GetByID($currentCatalog['BID'])) {
                    CCatalog::Add(['IBLOCK_ID' => $currentCatalog['BID']]);
                }
        ...
        case $nameUTF['Property']:
            if ($currentCatalog) {
                ...
                $currentProperty['Multiple'] = (('1' === $attrs[$nameUTF['Multiple']] || 'Y' === $attrs[$nameUTF['Multiple']]) ? 'Y' : 'N');
                $currentProperty['Name'] = $attrs[$nameUTF['Name']];
                $currentProperty['DefaultValue'] = $attrs[$nameUTF['DefaultValue']];

                if ('enumeration' === $currentProperty['DataType']) {
        ...
        case $nameUTF['Category']:
            if ($currentCatalog) {
                ...
                if (in_array($nameUTF['ParentCategory'], array_keys($attrs), true)) {
        ...
        case $nameUTF['Product']:
            if ($currentCatalog) {
                ...
                $strSql =
                    ...
                    "VALUES ('".$currentProduct['ID']."', ".$currentCatalog['BID'].", '".$DB->ForSql($currentProduct['Name'])."', ".(((int) $USER->GetID() > 0) ? (int) ($USER->GetID()) : 1).", '".$currentProduct['ParentCategory']."', '".(false === $currentProduct['Code'] ? '' : $DB->ForSql($currentProduct['Code']))."')";
        ...
        case $nameUTF['OffersList']:
            ...
            if (!array_key_exists($currentOffersList['CatalogID'], $arIBlockCache)) {
                $dbIBlockList = CIBlock::GetList([], ['XML_ID' => $currentOffersList['CatalogID']]);
                if ($arIBlock = $dbIBlockList->Fetch()) {
                    $arIBlockCache[$currentOffersList['CatalogID']] = (int) $arIBlock['ID'];
                }
            }
            ...
            $currentOffersList['ID'] = (int) $DB->LastID();
        ...
        case $nameUTF['Offer']:
            if ($currentOffersList) {
                ...
                $currentOffer['Price'] = (float) str_replace(',', '.', $attrs[$nameUTF['Price']]);
                $currentOffer['Amount'] = (int) $attrs[$nameUTF['Amount']];
                    ...
                    if ((int) $arRes['CNT'] <= 0) {
                        CCatalogGroup::Add(
                            [
                                ...
                                'USER_LANG' => ['ru' => $currentOffersList['PRICE_TYPE']],
                            ]
                        );
                    }
                }
...
function cmlCreateTempTables()
{
    ...
    $arTables = ['b_catalog_cml_tmp', 'b_catalog_cml_property', 'b_catalog_cml_property_var', 'b_catalog_cml_section', 'b_catalog_cml_product', 'b_catalog_cml_product_cat', 'b_catalog_cml_product_prop', 'b_catalog_cml_oflist', 'b_catalog_cml_oflist_prop', 'b_catalog_cml_offer'];
    ...
    while ($arRes = $dbRes->Fetch()) {
        ...
        if (in_array($tableName, $arTables, true)) {
    ...
    foreach ($arTables as $key => $value) {
        if ('b_catalog_cml_property' === $value) {
        ...
        } elseif ('b_catalog_cml_property_var' === $value) {
        ...
        } elseif ('b_catalog_cml_section' === $value) {
            $DB->Query('create table b_catalog_cml_section (
				...
				CODE varchar(255),
				primary key (CATALOG_ID, XML_ID))');
        } elseif ('b_catalog_cml_product' === $value) {
            $DB->Query('create table b_catalog_cml_product (
				...
				CODE varchar(255),
				primary key (CATALOG_ID, XML_ID))');
        } elseif ('b_catalog_cml_product_cat' === $value) {
        ...
        } elseif ('b_catalog_cml_product_prop' === $value) {
        ...
        } elseif ('b_catalog_cml_tmp' === $value) {
...
if ('' === $strImportErrorMessage) {
    ...
    if ('Y' !== $keepExistingProperties && 'N' !== $keepExistingProperties) {
        $keepExistingProperties = COption::GetOptionString('catalog', 'keep_existing_properties', CML_KEEP_EXISTING_PROPERTIES ? 'Y' : 'N');
    }
    $bKeepExistingProperties = (('Y' === $keepExistingProperties) ? true : false);

    if ('Y' !== $keepExistingData && 'N' !== $keepExistingData) {
    ...
    if ('Y' !== $activateFileData && 'N' !== $activateFileData) {
        $activateFileData = COption::GetOptionString('catalog', 'activate_file_data', CML_ACTIVATE_FILE_DATA ? 'Y' : 'N');
    }
    $bActivateFileData = (('Y' === $activateFileData) ? true : false);

    if ('Y' !== $deleteComments && 'N' !== $deleteComments) {
        $deleteComments = (CML_DELETE_COMMENTS ? 'Y' : 'N');
    }
    $bDeleteComments = (('Y' === $deleteComments) ? true : false);

    global $bCmlDebug;
    if ('Y' !== $cmlDebug && 'N' !== $cmlDebug) {
        $cmlDebug = (CML_DEBUG ? 'Y' : 'N');
    }
    $bCmlDebug = (('Y' === $cmlDebug) ? true : false);

    global $bCmlMemoryDebug;
    if ('Y' !== $cmlMemoryDebug && 'N' !== $cmlMemoryDebug) {
        $cmlMemoryDebug = (CML_MEMORY_DEBUG ? 'Y' : 'N');
    }
    $bCmlMemoryDebug = (('Y' === $cmlMemoryDebug) ? true : false);
    ...
    if ('0' !== $iBlockIDString) {
        ...
        if ('N' === $keepExistingData) {
            ...
            while ($arRes = $dbRes->Fetch()) {
                ...
                if ('0' !== $arPropsCache[$arRes['IBLOCK_ID']]) {
        ...
        while ($arRes = $dbRes->Fetch()) {
            $strSql =
                'UPDATE b_iblock_section SET '.
                '	IBLOCK_SECTION_ID = '.(((int) $arRes['VALUE_ID'] > 0) ? (int) ($arRes['VALUE_ID']) : 'NULL').' '.
                'WHERE ID = '.$arRes['ID'].' ';
            $DB->Query($strSql);
        }
        ...
        function ReSort($iblockID, $id = 0, $cnt = 0, $depth = 0, $active = 'Y')
        {
            ...
            if ($id > 0) {
                $DB->Query(
                    'UPDATE b_iblock_section SET '.
                    '	TIMESTAMP_X = '.(('ORACLE' === $DB->type) ? 'NULL' : 'TIMESTAMP_X').', '.
            ...
            $res = $DB->Query($strSql);
            while ($arr = $res->Fetch()) {
                $cnt = ReSort($iblockID, $arr['ID'], $cnt, $depth + 1, ('Y' === $active && 'Y' === $arr['ACTIVE']) ? 'Y' : 'N');
            }
        ...
        while ($arRes = $dbRes->Fetch()) {
            $strSql =
                ...
                "	NAME = '".$DB->ForSql($arRes['NAME'])."', ".
                '	IBLOCK_SECTION_ID = '.(((int) $arRes['PARENT_CATEGORY'] > 0) ? (int) ($arRes['PARENT_CATEGORY']) : 'NULL').', '.
                "	IN_SECTIONS = '".(((int) $arRes['CNT'] > 0) ? 'Y' : 'N')."', ".
                '	TIMESTAMP_X = NOW(), '.
                "	TMP_ID = '".$DB->ForSql($tmpid)."' ".
        ...
        if (!$bKeepExistingProperties && ('N' === $keepExistingData)) {
            $num = 0;
            $propertyIDs = '0';
